## German Grammar Cheatsheet (This content is based on teachings and guidance from **Mr. Talha Waheed Chauhan**)

---
> - **Last updated:**  16.03.2026 
> - **Access:**  Lifetime
---


> **📚 About This Cheatsheet**
> 
> This is an interactive, organized reference for German grammar concepts covering essential topics at **Absolute Beginner** (A1) and **Beginner** (A2) levels. Each section includes:
> - **Full Conjugation/Declension Tables**
> - **Quick Notes** with key concepts
> - **Memory Patterns** to help retention
> - **Example Sentences** for practical understanding

---

## 🎯 About us

<table>
<tr>
<td valign="top">



| Level | Description |
|-------|-------------|
| **Absolute Beginner** `Absolute Beginner` | A1 level concepts |
| **Beginner** `Beginner` | A2 level concepts |



</td>

<td valign="top">

**Written by**

| Sr. No.| Name | Profile |
|-------|-----|--------|
| 1 | Mr. Hassan Javed  M.Sc. | [link](https://github.com/RealisticHassan/German_language_Grammar_cheetsheet/blob/main/temp.md) |
| 2 | Ms. Mamoona Khan | [link](https://pubg.com/en) |


**Peer Reviewed by**

| Sr. No. | Name | Profile |
|-------|------|--------|
| 1 | Mr. Muhammad Adil Faisal | [link](https://www.linkedin.com/in/muhammad-adil-faisal-12817b108/) |
| 2 | Mr. Ahsan Ullah | [link](https://www.linkedin.com/in/ahsanullah512/) |
| 3 | Mr. Talha Waheed | [link](https://www.facebook.com/talha.w.chauhan) |
  


</td>
</tr>
</table>



## 📋 How to Use This Guide

| Feature | What You'll See |
|---------|-----------------|
| **► Triangle** | Click to expand/collapse sections |
| **Left Column** | Complete tables and conjugations |
| **Right Column** | Quick notes, patterns, and examples |
| **Blockquotes** | Important rules and tips highlighted |


## 🔤 Pronouns & Basic Grammar



<details >
<summary><b>📌 Personal Pronouns + haben + sein (Präsens)</b>| <code>Absolute Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Full Conjugation Table

| English | German | haben | sein |
|----------|--------|--------|--------|
| I | ich | habe | bin |
| You (informal) | du | hast | bist |
| He | er | hat | ist |
| She | sie | hat | ist |
| It | es | hat | ist |
| We | wir | haben | sind |
| You (plural) | ihr | habt | seid |
| You (formal) | Sie | haben | sind |
| They | sie | haben | sind |

</td>

<td valign="top">

### 🔹 Quick Notes

**Use of haben**
- Possession → Ich habe ein Auto  
- Age → Ich habe 25 Jahre  
- Feeling → Ich habe Hunger  
- Perfekt helper → Ich habe gearbeitet  

**Use of sein**
- Identity → Ich bin Student  
- Location → Wir sind in Wien  
- Condition → Er ist krank  
- Movement (Perfekt) → Ich bin gegangen  

**Structure**
Subject + Verb (2nd position) + Rest  

**Memory Pattern**

- haben → habe / hast / hat  
- plural → haben / habt / haben  

- sein → bin / bist / ist  
- plural → sind / seid / sind  

</td>
</tr>
</table>
</details>
 
<details>
<summary><b>📌 Präteritum (Past) – haben & sein</b> | <code>Absolute Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Full Conjugation Table

| Pronoun | haben (Past) | sein (Past) |
|----------|---------------|--------------|
| ich | hatte | war |
| du | hattest | warst |
| er | hatte | war |
| sie | hatte | war |
| es | hatte | war |
| wir | hatten | waren |
| ihr | hattet | wart |
| Sie | hatten | waren |
| sie | hatten | waren |

</td>

<td valign="top">

### 🔹 Quick Notes

**What is Präteritum?**  
Simple past tense (mainly written German).

**When is it used?**
- Stories → Er war müde.  
- News → Die Firma hatte Probleme.  
- Formal writing  
- Spoken: very common with *war* & *hatte*

**Key Structure**
 Subject + Präteritum Verb + Rest  

**Example:**
- Ich war krank.  
- Wir hatten keine Zeit.  

**When prefer over Perfekt?**
- Books, reports, newspapers  
- Modal verbs & haben/sein often in speech  

**Memory Pattern**

- hatte / hattest / hatte  
- hatten / hattet / hatten  

- war / warst / war  
- waren / wart / waren  

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Personal Pronouns – Nominative / Akkusative / Dative</b> | <code>Absolute Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Full Pronoun Table

| Nominative | Akkusative | Dative |
|-------------|------------|--------|
| ich | mich | mir |
| du | dich | dir |
| er | ihn | ihm |
| sie | sie | ihr |
| es | es | ihm |
| wir | uns | uns |
| ihr | euch | euch |
| Sie | Sie | Ihnen |
| sie | sie | ihnen |

</td>

<td valign="top">

### 🔹 Quick Notes

**Nominative (Subject)**  
→ Who is doing the action?  
**Example:**  
- Ich sehe dich.

**Akkusative (Direct Object)**  
→ Whom/what is affected?  
- Er sieht mich.  

**Dative (Indirect Object)**  
→ To whom? For whom?  
- Er gibt mir das Buch.  

**Common Verbs**
- sehen + Akk → Ich sehe ihn.  
- geben + Dat + Akk → Ich gebe dir das Buch.  

> **Key Structure**
>
> Subject (Nom) + Verb + Object (Akk/Dat)

**Example:**
- Ich gebe ihm einen Stift.  
- Wir besuchen sie.  

**Memory Pattern**

ich → mich → mir  
du → dich → dir  

er → ihn → ihm  

wir → uns → uns  
ihr → euch → euch  

Dative plural → often **-en** (ihnen)

</td>
</tr>
</table>

</details>



## 📝 Nouns, Articles & Modifiers



<details>
<summary><b>📌 Articles – Definite / Indefinite / Negation</b> | <code>Absolute Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Bestimmter Artikel (Definite)

| Case | Maskulin | Feminin | Neutrum | Plural |
|------|----------|----------|----------|---------|
| Nominativ | der | die | das | die |
| Akkusativ | den | die | das | die |
| Dativ | dem | der | dem | den (+n) |

---

### 🔹 Unbestimmter Artikel (Indefinite)

| Case | Maskulin | Feminin | Neutrum |
|------|----------|----------|----------|
| Nominativ | ein | eine | ein |
| Akkusativ | einen | eine | ein |
| Dativ | einem | einer | einem |

(Plural ❌ not used)

---

### 🔹 Negation Artikel (kein)

| Case | Maskulin | Feminin | Neutrum | Plural |
|------|----------|----------|----------|---------|
| Nominativ | kein | keine | kein | keine |
| Akkusativ | keinen | keine | kein | keine |
| Dativ | keinem | keiner | keinem | keinen (+n) |

</td>

<td valign="top">

### 🔹 Quick Notes

**Definite (der/die/das)**  
→ Specific thing  
- Der Mann ist hier.

**Indefinite (ein/eine)**  
→ Non-specific  
- Ich habe ein Auto.

**Negation (kein)**  
→ Means "no / not any"  
- Ich habe kein Auto.

---

### 🔹 Case Usage

**Nominativ** → Subject  
- Der Mann ist hier.

**Akkusativ** → Direct object  
- Ich sehe den Mann.

**Dativ** → Indirect object  
- Ich gebe dem Mann das Buch.

---

### 🔹 Important Patterns

> ✔ **Maskulin changes in Akkusativ** → **den / einen / keinen**  
> ✔ **Dativ plural** → **den + noun + n**  
> ✔ kein behaves like ein

---

### 🔹 Structure

> Subject (Nom) + Verb + Object (Akk/Dat)

**Example:**
- Ich sehe den Hund.  
- Ich gebe dem Kind einen Ball.  
- Ich habe keine Zeit.  

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Possessivartikel – Nominativ</b> | <code>Absolute Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Possessive Articles (Nominative)

| Pronoun | Maskulin (der) | Feminin (die) | Neutrum (das) | Plural (die) |
|----------|----------------|----------------|----------------|---------------|
| ich | mein | meine | mein | meine |
| du | dein | deine | dein | deine |
| er | sein | seine | sein | seine |
| sie | ihr | ihre | ihr | ihre |
| es | sein | seine | sein | seine |
| wir | unser | unsere | unser | unsere |
| ihr | euer | eure | euer | eure |
| Sie | Ihr | Ihre | Ihr | Ihre |
| sie | ihr | ihre | ihr | ihre |

</td>

<td valign="top">

### 🔹 Quick Notes

**What are Possessivartikel?**  
They show possession (my, your, his, her…).

**Example:**
- Das ist mein Auto.  
- Das ist meine Tasche.  

---

### 🔹 How They Work

✔ They behave like **ein-words**  
✔ Ending changes according to gender  

Pattern (Nominativ):

- Maskulin → no ending (mein Hund)  
- Feminin → -e (meine Tasche)  
- Neutrum → no ending (mein Kind)  
- Plural → -e (meine Freunde)

---

### 🔹 Structure

Possessive + Noun  

**Example:**
- Mein Bruder ist hier.  
- Meine Mutter arbeitet.  
- Unser Haus ist groß.  

---

### 🔹 Important Patterns

✔ ich → mein  
✔ du → dein  
✔ er/es → sein  
✔ sie (singular) → ihr  
✔ wir → unser  
✔ ihr → euer  
✔ Sie → Ihr  

**Remember:**
- euer → eure (drops extra e)

**Example:**
- eure Mutter  
(not *euere*)

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Possessivartikel – Akkusativ</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Possessive Articles (Akkusativ)

| Pronoun | Maskulin | Feminin | Neutrum | Plural |
|----------|------------|------------|------------|------------|
| ich | meinen | meine | mein | meine |
| du | deinen | deine | dein | deine |
| er | seinen | seine | sein | seine |
| sie | ihren | ihre | ihr | ihre |
| es | seinen | seine | sein | seine |
| wir | unseren | unsere | unser | unsere |
| ihr | euren | eure | euer | eure |
| Sie | Ihren | Ihre | Ihr | Ihre |
| sie | ihren | ihre | ihr | ihre |

</td>

<td valign="top">

### 🔹 Quick Notes

**What changes in Akkusativ?**

✔ Only **Maskulin** changes → gets **-en**

Pattern:
- meinen Hund  
- deinen Bruder  
- seinen Vater  

Feminin / Neutrum / Plural  
→ same as Nominativ

---

### 🔹 When do we use Akkusativ?

Direct object (Wen? Was?)

**Example:**
- Ich sehe meinen Bruder.  
- Ich kaufe meine Tasche.  
- Wir besuchen unsere Freunde.  

---

### 🔹 Structure

Subject + Verb + Possessive + Noun  

**Example:**
- Ich liebe meinen Hund.  
- Sie sucht ihre Tasche.  

---

### 🔹 Important Pattern

Nominativ → mein Hund  
Akkusativ → meinen Hund  

Only masculine changes!

✔ ein-word rule still applies  
✔ euer → euren (drops extra e)

**Example:**
- euren Vater  
(not *eueren*)

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 Possessivartikel – Dativ</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Possessive Articles (Dativ)

| Pronoun | Maskulin | Feminin | Neutrum | Plural |
|----------|------------|------------|------------|------------|
| ich | meinem | meiner | meinem | meinen (+n) |
| du | deinem | deiner | deinem | deinen (+n) |
| er | seinem | seiner | seinem | seinen (+n) |
| sie | ihrem | ihrer | ihrem | ihren (+n) |
| es | seinem | seiner | seinem | seinen (+n) |
| wir | unserem | unserer | unserem | unseren (+n) |
| ihr | eurem | eurer | eurem | euren (+n) |
| Sie | Ihrem | Ihrer | Ihrem | Ihren (+n) |
| sie | ihrem | ihrer | ihrem | ihren (+n) |

</td>

<td valign="top">

### 🔹 Quick Notes

**What changes in Dativ?**

✔ Maskulin → -em  
✔ Neutrum → -em  
✔ Feminin → -er  
✔ Plural → -en (+ noun + n)

---

### 🔹 When do we use Dativ?

Indirect object (Wem?)

**Example:**
- Ich gebe meinem Bruder das Buch.  
- Sie hilft ihrer Mutter.  
- Wir sprechen mit unseren Freunden.  

---

### 🔹 Structure

Subject + Verb + Dativ object + Akkusativ object  

**Example:**
- Ich gebe meinem Vater einen Stift.  

---

### 🔹 Important Patterns

Nominativ → mein Vater  
Akkusativ → meinen Vater  
Dativ → meinem Vater  

Plural:
- mit meinen Freunden  
(n gets added to noun if possible)

✔ euer → eurem / euren  
(not *euerm*)

</td>
</tr>
</table>

</details>

 

## 🔀 Verb Forms & Tenses

 

<details>
<summary><b>📌 Modalverben – Präsens</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Conjugation Table (Präsens)

| Pronoun | können | müssen | wollen | sollen | dürfen | möchten |
|----------|----------|----------|----------|----------|----------|-----------|
| ich | kann | muss | will | soll | darf | möchte |
| du | kannst | musst | willst | sollst | darfst | möchtest |
| er/sie/es | kann | muss | will | soll | darf | möchte |
| wir | können | müssen | wollen | sollen | dürfen | möchten |
| ihr | könnt | müsst | wollt | sollt | dürft | möchtet |
| Sie/sie | können | müssen | wollen | sollen | dürfen | möchten |

</td>

<td valign="top">

### 🔹 Quick Notes

**What are Modalverben?**  
They modify another verb (ability, obligation, permission, desire).

---

### 🔹 Sentence Structure

Subject + Modalverb + ... + Main Verb (infinitive at end)

**Example:**
- Ich kann Deutsch sprechen.  
- Du musst heute arbeiten.  
- Wir wollen Pizza essen.  

---

### 🔹 Important Rules

> ✔ Main verb goes to the END  
> ✔ ich & er/sie/es → same form  
> ✔ No "zu" with modal verbs  

❌ Wrong: Ich kann zu schwimmen.  
✅ Correct: Ich kann schwimmen.  

---

### 🔹 Meaning Overview

- können → can / be able to  
- müssen → must / have to  
- wollen → want to  
- sollen → should / supposed to  
- dürfen → may / allowed to  
- möchten → would like  

---

### 🔹 Memory Pattern

**Singular core forms:**
- kann / muss / will / soll / darf  

**Plural:**
- können / müssen / wollen / sollen / dürfen  

möchten behaves like weak verb.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Modalverben – Präteritum (Past)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Conjugation Table (Präteritum)

| Pronoun | können | müssen | wollen | sollen | dürfen | möchten* |
|----------|---------|---------|---------|---------|---------|----------|
| ich | konnte | musste | wollte | sollte | durfte | wollte |
| du | konntest | musstest | wolltest | solltest | durftest | wolltest |
| er/sie/es | konnte | musste | wollte | sollte | durfte | wollte |
| wir | konnten | mussten | wollten | sollten | durften | wollten |
| ihr | konntet | musstet | wolltet | solltet | durftet | wolltet |
| Sie/sie | konnten | mussten | wollten | sollten | durften | wollten |

*möchten → no own Präteritum (use wollte)*

</td>

<td valign="top">

### 🔹 Usage Notes

✔ Präteritum is very common with modal verbs  
✔ Especially in written German  
✔ Very often used instead of Perfekt

---

### 🔹 Example Sentences

- Ich konnte nicht kommen.  
- Wir mussten lange arbeiten.  
- Er wollte Arzt werden.  
- Du solltest mehr lernen.  
- Sie durften nicht hinein.

---

### 🔹 Structure

Subject + Modalverb (Präteritum) + Infinitive (END)

**Example:**

Ich musste gestern arbeiten.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Modalverben – Perfekt</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Structure (IMPORTANT ⚠)

Perfekt with modal verbs uses:

haben + Infinitiv + Modalverb (Infinitiv)

NOT Partizip II!

Structure:

Subject + haben + ... + Main Verb (Infinitive) + Modalverb (Infinitive)

---

### 🔹 Example Pattern

Ich habe arbeiten müssen.  
Wir haben gehen können.  
Er hat kommen wollen.

</td>

<td valign="top">

### 🔹 Examples

- Ich habe Deutsch sprechen können.  
- Du hast lange warten müssen.  
- Wir haben früher gehen dürfen.  
- Er hat Arzt werden wollen.  
- Sie hat mehr lernen sollen.

---

### 🔹 Important Rule

> Modal verbs in Perfekt:
> 
> ❌ NO "ge-"  
> ❌ NO Partizip II  
> 
> ✅ Ich habe arbeiten müssen.  

Wrong examples:
- ❌ Ich habe gemusst.  
- ❌ Ich habe gekonnt.  

---

### 🔹 Spoken German Tip

**In spoken German:**

Präteritum is more common for modal verbs.

**Instead of:**
Ich habe arbeiten müssen.

**People often say:**
Ich musste arbeiten.

</td>
</tr>
</table>

</details>




<details>
<summary><b>📌 Perfekt mit Partizip II – Present Perfect (German Past Tense)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What is Perfekt?

Perfekt is a **past tense** used in spoken German.

English examples

- I played cricket  
- I have played cricket  
- I cooked food  

German

- Ich habe Kriket gespielt.  
- Ich habe Biryani gekocht.

---

### 🔹 Structure of Perfekt

Perfekt = **haben / sein + Partizip II**

Sentence pattern

Subject + **haben / sein** + ... + **Partizip II (end)**

Example

Ich **habe** Medizin **studiert**.  
Paula **hat** ein Handy **gekauft**.

---

### 🔹 Sentence Position Rule

| Position 1 | Position 2 | Middle | Sentence End |
|---|---|---|---|
| Subject | haben / sein | Object / Time | Partizip II |

Example

Ich | habe | Medizin | studiert.

---

### 🔹 Question Structure

Verb comes first.

Example

Hat Paula ein Handy gekauft?

Example with question word

Was haben Sie an der Universität studiert?

Wer hat an der Universität studiert?

</td>

<td valign="top">

### 🔹 Partizip II Formation (Regular Verbs)

Rule

**ge + verb stem + t**

Examples

| Verb | Partizip II |
|---|---|
| lernen | gelernt |
| hören | gehört |
| machen | gemacht |
| kochen | gekocht |
| kaufen | gekauft |
| tanzen | getanzt |

Examples

Ich habe Deutsch gelernt.  
Ich habe die Übung gemacht.  

---

### 🔹 Verbs ending with **-ieren**

These verbs **DO NOT use "ge-"**

Pattern

Verb + **t**

Examples

| Verb | Perfekt |
|---|---|
| studieren | hat studiert |
| trainieren | hat trainiert |
| fotografieren | hat fotografiert |
| buchstabieren | hat buchstabiert |
| reagieren | hat reagiert |

Examples

Ich habe Englisch studiert.  
Sie hat im Park fotografiert.

</td>
</tr>

<tr>
<td valign="top">

### 🔹 haben vs sein

Most verbs use **haben**

Examples

Ich habe gelernt.  
Ich habe gearbeitet.  

---

### 🔹 Use **sein** with

1️⃣ movement  
2️⃣ change of state

Examples

| Verb | Perfekt |
|---|---|
| kommen | ist gekommen |
| fahren | ist gefahren |
| laufen | ist gelaufen |
| schwimmen | ist geschwommen |

Example

Alle meine Freunde **sind** zur Party **gekommen**.

---

### 🔹 More Example Sentences

Ich habe heute 2 Stunden trainiert.  
Sie hat im Park fotografiert.  
Wir haben am Computer recherchiert.  
Der Schauspieler hat lustig reagiert.

</td>

<td valign="top">

### 🔹 Translation Example

Paula hat ein Handy gekauft.

Meanings

• Paula bought a mobile  
• Paula has bought a mobile  

Urdu meaning

Paula ne mobile khareeda.

---

### 🔹 More Examples

Ich habe Kriket gespielt.  
Ich habe Biryani gekocht.  

Gestern habe ich um 12 Uhr zu Hause die Übung gemacht.

Ich habe um 12 Uhr die Übung gemacht.

</td>
</tr>
</table>

</details>



<details>
<summary><b>📌 Unregelmäßige Verben – Irregular Verbs (Perfekt)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Irregular Verbs?

Some verbs **do not follow the normal "ge + t" rule**.

Their **Partizip II changes form**.

Example

Er liest ein Buch.  
Er hat ein Buch gelesen.

---

### 🔹 Common Irregular Verbs

| Verb | Perfekt |
|---|---|
| lesen | hat gelesen |
| sehen | hat gesehen |
| geben | hat gegeben |
| kommen | ist gekommen |
| fahren | ist gefahren |
| backen | hat gebacken |
| schlafen | hat geschlafen |
| laufen | ist gelaufen |

---

### 🔹 Exceptions

| Verb | Perfekt |
|---|---|
| essen | hat gegessen |
| gehen | ist gegangen |

</td>

<td valign="top">

### 🔹 Vowel Change Verbs

Some verbs change their vowel.

| Verb | Perfekt |
|---|---|
| nehmen | hat genommen |
| sprechen | hat gesprochen |
| finden | hat gefunden |
| trinken | hat getrunken |
| schwimmen | ist geschwommen |
| fliegen | ist geflogen |
| helfen | hat geholfen |
| stehen | hat gestanden |
| singen | hat gesungen |
| sitzen | hat gesessen |
| wissen | hat gewusst |
| kennen | hat gekannt |
| denken | hat gedacht |

---

### 🔹 More Exceptions

| Verb | Perfekt |
|---|---|
| schreiben | hat geschrieben |
| bleiben | ist geblieben |

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 Ordinalzahlen – Ordinal Numbers (erste, zweite, dritte...)</b> | <code>Absolute Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Ordinalzahlen?

Ordinal numbers show **order or position**.

Examples

1st → first  
2nd → second  
3rd → third  

German

erste, zweite, dritte

---

### 🔹 Formation Rule

| Numbers | Ending |
|---|---|
| 1 – 19 | **-te** |
| 20+ | **-ste** |

Examples

| Number | Ordinal |
|---|---|
| eins | erste |
| zwei | zweite |
| drei | dritte |
| vier | vierte |
| fünf | fünfte |
| sechs | sechste |
| sieben | siebte |
| acht | achte |
| neun | neunte |
| zehn | zehnte |

</td>

<td valign="top">

### 🔹 More Examples

| Number | Ordinal |
|---|---|
| elf | elfte |
| zwölf | zwölfte |
| dreizehn | dreizehnte |
| neunzehn | neunzehnte |
| zwanzig | zwanzigste |
| hundert | hundertste |

---

### 🔹 Question used

**Welches Datum haben wir heute?**

Example answers

Heute ist **der zweite März**.  
Heute ist **der fünfundzwanzigste Dezember**.  
Heute ist **der zweite November**.

---

### 🔹 Date Pattern

German Date

**der + ordinal number + month**

Example

02.03  
Heute ist **der zweite März**.

25.12  
Heute ist **der fünfundzwanzigste Dezember**.

</td>
</tr>
</table>

</details>



<details>
<summary><b>📌 Dativ Artikel – German Dative Articles</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Bestimmter Artikel (Definite)

| Gender | Nominativ | Akkusativ | Dativ |
|---|---|---|---|
| Maskulin | der | den | dem |
| Neutrum | das | das | dem |
| Feminin | die | die | der |
| Plural | die | die | den + n |

**Example**

Der Mann isst den Apfel.

Die Pizza ist **für den Mann**.

</td>

<td valign="top">

### 🔹 Unbestimmter Artikel

| Gender | Nominativ | Akkusativ | Dativ |
|---|---|---|---|
| Maskulin | ein | einen | einem |
| Neutrum | ein | ein | einem |
| Feminin | eine | eine | einer |
| Plural | — | — | — |

**Example**

Ich spreche **mit einem Mann**.

</td>
</tr>
<tr>
<td>

</td>
<td>

### 🔹 Negation Artikel (kein)

| Gender | Nominativ | Akkusativ | Dativ |
|---|---|---|---|
| Maskulin | kein | keinen | keinem |
| Neutrum | kein | kein | keinem |
| Feminin | keine | keine | keiner |
| Plural | keine | keine | keinen + n |

**Example**

Ich habe **keinen Freund**.
</td>
</tr>
</table>
</details>



<details>
<summary><b>📌 Präpositionen – Wo / Woher / Wohin</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Question Words

| Question | Meaning |
|---|---|
| Wo | where |
| Woher | where from |
| Wohin | where to |

---

### 🔹 in (location)

Used for **inside a place**

Examples

Ich bin **in der Schule**.  
Ich arbeite **in der Fabrik**.  
Wir treffen uns **im Park**.

(im = in dem)

---

### 🔹 bei + Dativ

Meaning

• at someone's place  
• near

Examples

Ich wohne **bei meinen Eltern**.  
Ich arbeite **bei der Firma Fischer**.

Near meaning

Ich wohne **beim Bahnhof**.


---

### 🔹 Short Forms

| Full | Short |
|---|---|
| zu dem | zum |
| zu der | zur |
| bei dem | beim |
| von dem | vom |
| in dem | im |
 
</td>

<td valign="top">

### 🔹 aus (Woher – from)

Used for origin.

Examples

Er kommt **aus Deutschland**.  
Er kommt **aus München**.

Countries with article

Ich komme **aus der Schweiz**.  
Ich komme **aus den USA**.

---

### 🔹 von (from person/place)

Examples

Er kommt **von Barbara**.  
Er kommt **vom Arzt**.  
Er kommt **vom Bahnhof**.

(vom = von dem)

---

### 🔹 nach (Wohin – to country/city)

Examples

Ich fliege **nach Mexiko**.  
Ich fahre **nach Köln**.  
Ich gehe **nach Hause**.

Exception (countries with article)

Ich fahre **in die Türkei**.  
Ich fliege **in die USA**.

---

### 🔹 zu + Dativ (to a person/place)

Examples

Ich gehe **zu Barbara**.  
Ich gehe **zum Supermarkt**.  
Ich gehe **zur Schule**.



</td>
</tr>
 

</table>

</details>


## 🎯 Sentence Structure & Clauses

 

<details>
<summary><b>📌 dass – Nebensatz (Subordinate Clause)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Structure Overview

| Position | Example |
|----------|----------|
| Hauptsatz + , + dass + Subject + ... + Verb (END) | Ich bin glücklich, dass ich Deutsch sprechen kann. |
| dass-clause first | Dass Köln schön ist, weiß ich. |

✔ Verb goes to LAST position  
✔ Comma before „dass“  

---

### 🔹 Typical Verbs Before „dass“

- denken  
- glauben  
- wissen  
- meinen  
- finden  
- hoffen  

**Example:**
- Ich glaube, dass er recht hat.  
- Wir wissen, dass du müde bist.  

---

### 🔹 Expressions with "es"

- Es freut mich, dass du kommst.  
- Es ist schön, dass du hier bist.  
- Es tut mir leid, dass ich nicht kommen kann.  

</td>

<td valign="top">

### 🔹 What is a „dass“-Satz?

✔ „dass“ = that  
✔ It introduces a subordinate clause (Nebensatz)  
✔ The conjugated verb goes to the END  

Rule:
Nebensatz beginnt mit Konnektor und endet mit Verb.

---

### 🔹 Important Rules

> ✔ Comma between main and subordinate clause  
> ✔ Subordinate clause can come before or after main clause  
> ✔ Separable verbs are NOT separated in subordinate clause  

**Example:**
Ich weiß, dass er morgen ankommt.  
(not: kommt an)

---

### 🔹 Spoken German Note

In spoken German, „dass“ is often omitted.

Formal:
Viele finden, dass Köln eine schöne Stadt ist.  

Spoken:
Viele finden, Köln ist eine schöne Stadt.  

---

### 🔹 Structure Formula

Main clause + , + dass + Subject + Object + Verb  

OR  

dass + Subject + Object + Verb , Main clause  

---

### 🔹 Example Sentences

- Ben hofft, dass er alles versteht.  
- Ich denke, dass du recht hast.  
- Dass er krank ist, weiß ich.  

</td>
</tr>
</table>

</details>











<details>
<summary><b>📌 weil – Subordinate Clause (because)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Structure

Main clause + , + weil + Subject + ... + Verb (END)

| Example |
|----------|
| Ich lerne Deutsch, weil ich in Deutschland studiere. |
| Er bleibt zu Hause, weil er krank ist. |
| Ich bin müde, weil ich viel gearbeitet habe. |

✔ Conjugated verb goes to the END  
✔ Comma before "weil"

---

### 🔹 weil-Clause First

weil + Subject + ... + Verb (END), Main clause

| Example |
|----------|
| Weil ich krank bin, bleibe ich zu Hause. |
| Weil es regnet, bleiben wir drinnen. |

⚠ When weil comes first → main verb in position 2

---

### 🔹 Answering "Warum?"

You can answer directly with a weil-sentence.

Warum lernst du Deutsch?  
→ Weil ich in Deutschland leben möchte.

Warum kommst du nicht?  
→ Weil ich keine Zeit habe.

</td>

<td valign="top">

### 🔹 What is a „weil“-Satz?

✔ „weil“ = because  
✔ It expresses a reason  
✔ It introduces a subordinate clause  
✔ Verb goes to the END  

---

### 🔹 Important Rules

> ✔ Comma between main clause and weil clause  
> ✔ weil clause can come before or after main clause  
> ✔ In subordinate clause, separable verbs are NOT separated  

**Example:**
Ich bleibe zu Hause, weil ich morgen früh aufstehen muss.

---

### 🔹 Spoken German Note

In informal spoken German, sometimes people use:
"... weil er hat keine Zeit."

⚠ Grammatically correct standard German:
"... weil er keine Zeit hat."

---

### 🔹 Structure Formula

Reason after:
Main clause + weil + subject + object + verb  

Reason first:
weil + subject + object + verb , main clause  

---

### 🔹 More Examples

- Ich spare Geld, weil ich ein Auto kaufen möchte.  
- Wir fahren mit dem Bus, weil unser Auto kaputt ist.  
- Er war nervös, weil er einen Unfall hatte.  
- Ich gehe nicht zur Arbeit, weil ich krank bin.  

</td>
</tr>
</table>

</details>





<details>
<summary><b>📌 wenn – Subordinate Clause (if / when)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Structure

wenn + Subject + ... + Verb (END), Main clause

| Example |
|----------|
| Wenn ich Zeit habe, besuche ich meine Familie. |
| Wenn es regnet, bleiben wir zu Hause. |

✔ Verb goes to the END in the wenn-clause  
✔ Comma separates both clauses  

---

### 🔹 wenn After Main Clause

Main clause + , + wenn + Subject + ... + Verb (END)

| Example |
|----------|
| Ich bleibe zu Hause, wenn ich krank bin. |
| Ich bin glücklich, wenn ich Geburtstag habe. |

---

### 🔹 Common Pattern with "dann"

Wenn + condition, dann + result

**Example:**
- Wenn ich Geburtstag habe, dann feiere ich.  
- Wenn ich zu spät komme, dann entschuldige ich mich.  

("dann" is optional)

</td>

<td valign="top">

### 🔹 What is a „wenn“-Satz?

✔ „wenn“ = if / when  
✔ It expresses a condition or repeated situation  
✔ It introduces a subordinate clause  
✔ Verb goes to the END  

---

### 🔹 When Do We Use „wenn“?

1️⃣ Conditions (if)  
- Wenn ich krank bin, bleibe ich zu Hause.

2️⃣ Repeated situations (whenever)  
- Wenn ich müde bin, trinke ich Kaffee.

3️⃣ Future conditions  
- Wenn ich Urlaub habe, reise ich nach Pakistan.

---

### 🔹 Important Rules

> ✔ Comma between clauses  
> ✔ wenn-clause can come first or second  
> ✔ If wenn-clause is first → main verb in position 2  

**Example:**
Wenn ich Zeit habe, helfe ich dir.  
(NOT: helfe dir ich)

---

### 🔹 Question & Answer Pattern

Question:
Was machst du, wenn du Urlaub hast?

Answer:
Wenn ich Urlaub habe, besuche ich meine Familie.

---

### 🔹 More Examples

- Wenn ich Kopfschmerzen habe, gehe ich früh ins Bett.  
- Wenn das Wetter schön ist, machen wir einen Ausflug.  
- Wenn ich schlechte Laune habe, höre ich Musik.  
- Wenn ich zur Hochzeit gehe, kaufe ich ein Geschenk.  

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 damit – Nebensatz (Purpose Clause)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Structure Overview

| Position | Example |
|----------|----------|
| Hauptsatz + , + damit + Subject + ... + Verb (END) | Ich lerne Deutsch, damit ich in Deutschland arbeiten kann. |
| damit-clause first | Damit ich Deutsch lerne, gehe ich jeden Tag zum Kurs. |

✔ Verb goes to LAST position  
✔ Comma before „damit“

---

### 🔹 Typical Verbs Before „damit“

Often used with actions done **for a purpose**

- lernen  
- arbeiten  
- sparen  
- üben  
- kaufen  

**Example:**

- Ich lerne viel, damit ich die Prüfung bestehe.  
- Wir sparen Geld, damit wir ein Haus kaufen können.

---

### 🔹 Common Situations

✔ expressing **purpose / goal**

- Ich rufe dich an, damit du Bescheid weißt.  
- Er spricht langsam, damit alle ihn verstehen.

</td>

<td valign="top">

### 🔹 What is a „damit“-Satz?

✔ „damit“ = **so that / in order that**  
✔ It introduces a **purpose clause (Zweck)**  
✔ The conjugated verb goes to the **END**

Rule:

Nebensatz beginnt mit **damit** und endet mit **Verb**.

---

### 🔹 Important Rules

> ✔ Comma between main and subordinate clause  
> ✔ Verb goes to the end  
> ✔ Subject can be different in both clauses

Example:

Ich erkläre es langsam, damit du es verstehst.

---

### 🔹 damit vs um...zu

| damit | um...zu |
|------|------|
| subject different ho sakta hai | subject same hota hai |
| full clause hoti hai | infinitive structure |

Example:

Ich lerne Deutsch, damit ich in Deutschland arbeiten kann.  
Ich lerne Deutsch, **um in Deutschland zu arbeiten.**

---

### 🔹 Structure Formula

Main clause + , + damit + Subject + Object + Verb

OR

damit + Subject + Object + Verb , Main clause

---

### 🔹 Example Sentences

- Ich spreche laut, damit du mich hörst.  
- Sie lernt viel, damit sie die Prüfung besteht.  
- Damit ich fit bleibe, mache ich jeden Tag Sport.

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 werden – Verb (to become / future helper)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Meaning

werden = to become / to turn into  

It is used:
- with nouns → Er wird Arzt.
- with adjectives → Das Wetter wird kalt.
- for age → Ich werde 30 Jahre alt.
- for future tense

---

### 🔹 Präsens (Present)

| Pronoun | werden |
|----------|----------|
| ich | werde |
| du | wirst |
| er/sie/es | wird |
| wir | werden |
| ihr | werdet |
| Sie/sie | werden |

**Example:**
- Ich werde Lehrer.
- Du wirst krank.

---

### 🔹 Präteritum (Simple Past)

| Pronoun | wurden |
|----------|----------|
| ich | wurde |
| du | wurdest |
| er/sie/es | wurde |
| wir | wurden |
| ihr | wurdet |
| Sie/sie | wurden |

**Example:**
- Er wurde Arzt.
- Das Wetter wurde kalt.

---

### 🔹 Perfekt (Present Perfect)

Structure: sein + geworden

| Pronoun | Perfekt |
|----------|----------|
| ich | bin geworden |
| du | bist geworden |
| er/sie/es | ist geworden |
| wir | sind geworden |
| ihr | seid geworden |
| Sie/sie | sind geworden |

**Example:**
- Sie ist Ärztin geworden.
- Ich bin krank geworden.

</td>

<td valign="top">

### 🔹 Usage Explanation

1️⃣ Becoming something  
- Er wird Ingenieur.
- Sie ist Ärztin geworden.

2️⃣ Change of state  
- Das Wetter wird schön.
- Die Suppe ist kalt geworden.

3️⃣ Age  
- Ich werde 27 Jahre alt.
- Letztes Jahr bin ich 26 geworden.

4️⃣ Future Tense (Helper Verb)

Structure:
Subject + werden + ... + Infinitive (at end)

**Example:**
- Ich werde morgen arbeiten.
- Wir werden Deutsch lernen.

---

### 🔹 Important Notes

✔ Perfekt uses **sein**, not haben  
✔ geworden (NOT *gewordt*)  
✔ For future: main verb goes to END  
✔ In spoken German, Präsens often replaces future  

**Example:**
Morgen gehe ich nach Berlin.  
(Instead of: Ich werde morgen nach Berlin gehen.)

---

### 🔹 Structure Summary

Becoming:
Subject + werden + Noun/Adjective  

Future:
Subject + werden + Infinitive (END)

Perfekt:
Subject + sein + geworden  

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 Indirekte Fragesätze (Indirect Questions)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What is an Indirect Question?

Indirect questions are polite forms of questions.  
They are used after phrases like:

- Ich möchte wissen, …
- Können Sie mir sagen, …
- Wissen Sie, …
- Ich weiß nicht, …

They are subordinate clauses → the verb goes to the END.

---

## 🔹 1️⃣ Yes/No Questions → "ob"

If the direct question is a yes/no question,  
the indirect question begins with **ob**.

### Structure

Main clause + , + ob + Subject + ... + Verb (END)

| Direct Question | Indirect Question |
|----------------|------------------|
| Kommt er heute? | Ich weiß nicht, ob er heute kommt. |
| Ist der Film gut? | Können Sie mir sagen, ob der Film gut ist? |

---

## 🔹 2️⃣ W-Questions

If the direct question begins with a W-word,  
the indirect question keeps the SAME W-word.

### Structure

Main clause + , + W-word + Subject + ... + Verb (END)

| Direct Question | Indirect Question |
|----------------|------------------|
| Wo ist der Bahnhof? | Ich möchte wissen, wo der Bahnhof ist. |
| Wann beginnt der Film? | Weißt du, wann der Film beginnt? |

</td>

<td valign="top">

### 🔹 Important Rules

✔ Indirect questions are subordinate clauses  
✔ The conjugated verb goes to the END  
✔ Separable verbs are NOT separated  

**Example:**
Ich weiß nicht, wann er ankommt.  
(not: kommt an)

---

### 🔹 Punctuation Rules

If the introductory clause is NOT a question → use a period (.)

Ich möchte wissen, wann er kommt.  

If the introductory clause IS a question → use a question mark (?)

Weißt du, wann er kommt?  

---

### 🔹 More Examples

- Ich habe keine Ahnung, ob das Geschäft geöffnet ist.  
- Können Sie mir sagen, wo der Bahnhof ist?  
- Ali fragt, wann der nächste Bus kommt.  
- Ich weiß nicht, warum der Zug spät ist.  
- Sag mir bitte, ob wir heute Prüfung haben.  

---

### 🔹 Structure Summary

Yes/No:
Main clause + ob + subject + verb (END)

W-question:
Main clause + W-word + subject + verb (END)

Verb always at the END.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Adjektivdeklination nach unbestimmten Artikel</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Adjektivdeklination nach unbestimmten Artikel?

- In the nominative and accusative singular after the indefinite article, the adjective always takes the ending of the definite article.

- Therefore, the adjective must take over the missing ending → strong ending.

- When the indefinite article, negation, or possessive article has a declension (an ending), the adjective receives a weak ending, i.e., “-e” or “-en”.

In the dative singular and plural, the adjective has the ending “-en”.

When an adjective comes after:

- ein
- kein
- mein / dein / sein / ihr / unser / euer / Ihr


the adjective must take an ending.

Because:
The indefinite article does NOT always show full case information.
So the adjective sometimes takes the missing ending.

---

### 🔹 1️⃣ Nominative

| Gender | Article | Adjective Ending | Example |
|--------|----------|-----------------|----------|
| Masculine | ein | -er | ein **neuer** Computer |
| Neuter | ein | -es | ein **schönes** Haus |
| Feminine | eine | -e | eine **freundliche** Frau |
| Plural (kein/meine) | keine | -en | keine **alten** Autos |

---

### 🔹 2️⃣ Accusative

| Gender | Article | Adjective Ending | Example |
|--------|----------|-----------------|----------|
| Masculine | einen | -en | einen **großen** Hund |
| Neuter | ein | -es | ein **kleines** Kind |
| Feminine | eine | -e | eine **nette** Frau |
| Plural (kein/meine) | keine | -en | keine **schweren** Kisten |


### 🔹 3️⃣ Dative

In Dative → adjective almost always ends in **-en**

| Gender | Article | Adjective Ending | Example |
|--------|----------|-----------------|----------|
| Masculine | einem | -en | einem **netten** Mann |
| Neuter | einem | -en | einem **kleinen** Kind |
| Feminine | einer | -en | einer **alten** Frau |
| Plural | keinen/meinen | -en | keinen **alten** Autos |

---

</td>

<td valign="top">


### 🔹 Important Rules

✔ Masculine Accusative → always **-en**  
✔ Dative → always **-en**  
✔ Neuter Nom/Akk → **-es**  
✔ Feminine Nom/Akk → **-e**

---

### 🔹 Easy Memory Trick

Think:

• der → -er  
• das → -es  
• die → -e  

If article shows strong ending → adjective becomes weaker  
If article is missing ending → adjective becomes stronger  

---

### 🔹 Example Sentences

#### Nominative
- Ein junger Mann wartet.
- Eine freundliche Frau hilft.
- Mein neuer Laptop funktioniert gut.

#### Accusative
- Ich sehe einen großen Hund.
- Wir kaufen ein frisches Brot.
- Ich brauche meinen neuen Schlüssel.

#### Dative
- Ich helfe einem netten Kollegen.
- Sie dankt einer geduldigen Lehrerin.
- Ich vertraue meinem besten Freund.

---

### 🔹 Structure Reminder

Article + Adjective + Noun

**Example:**
ein + groß + Hund  
→ einen großen Hund

Adjective ending depends on:
Gender + Case + Article type

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Konjunktiv II – sollte (sollen)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

#### 🔹 What is "sollte"?

Konjunktiv II of **sollen** = **should**

It is used for:

- Polite advice  
- Recommendations  
- Soft suggestions  
- Indirect expectations  

It sounds softer and less direct than normal "sollen".

---

### 🔹 Conjugation

| Pronoun | Form |
|----------|--------|
| ich | sollte |
| du | solltest |
| er/sie/es | sollte |
| wir | sollten |
| ihr | solltet |
| sie/Sie | sollten |

---

### 🔹 Sentence Structure

Position 1 | Position 2 | Rest | Verb (Infinitive at End)
---|---|---|---
Du | solltest | mehr | lernen.
Ihr | solltet | langsamer | fahren.

**Structure:**

Subject + sollte(n/st) + ... + Infinitive (END)

**Example:**
Du solltest mehr trainieren.

</td>

<td valign="top">

### 🔹 1️⃣ Polite Advice (Höfliche Ratschläge)

"sollte" makes advice softer and respectful.

✔ Du solltest mehr Wasser trinken.  
✔ Du solltest früher schlafen gehen.  
✔ Ihr solltet pünktlich kommen.

---

### 🔹 2️⃣ Indirect Expectation

It shows something is expected,  
but it is not 100% certain.

✔ Der Zug sollte um 18 Uhr kommen.  
(We expect it, but not sure.)

✔ Er sollte eigentlich schon hier sein.  

---

### 🔹 Difference

| sollen (Indikativ) | sollte (Konjunktiv II) |
|--------------------|------------------------|
| Du sollst lernen. | Du solltest lernen. |
| (stronger order) | (soft advice) |

---

### 🔹 With weil / Nebensatz

Du solltest die Zeitschrift lesen,  
weil sie sehr informativ ist.

Du solltest deine Aufgaben machen,  
weil du bald Prüfung hast.

---

### 🔹 Quick Memory

"sollte" = soft recommendation  
Sounds polite  
Common in spoken German  

Very important for B1 level.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Konjunktiv II – könnte (können) Part I</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

#### 🔹 What is "könnte"?

Konjunktiv II of **können** = **could**

It is used for:

- Polite requests  
- Suggestions  
- Possibilities  
- Hypothetical situations  

It sounds softer and more polite than "kann".

---

#### 🔹 Conjugation

| Pronoun | Form |
|----------|--------|
| ich | könnte |
| du | könntest |
| er/sie/es | könnte |
| wir | könnten |
| ihr | könntet |
| sie/Sie | könnten |

---

#### 🔹 Sentence Structure

Position 1 | Position 2 | Rest | Infinitive (End)
---|---|---|---
Du | könntest | mir | helfen.
Wir | könnten | heute | gehen.

Structure:

Subject + könnte(n/st) + ... + Infinitive (END)

**Example:**
Du könntest mehr lernen.

</td>

<td valign="top">

#### 🔹 1️⃣ Polite Request

Very common in formal German.

✔ Könnten Sie mir helfen?  
✔ Könntest du mir bitte das Salz geben?  

More polite than:
Kannst du mir helfen?

---

#### 🔹 2️⃣ Suggestion

✔ Du könntest früher schlafen gehen.  
✔ Wir könnten ins Kino gehen.  

It means: "It would be a good idea."

---

#### 🔹 3️⃣ Possibility (Not Certain)

✔ Es könnte morgen regnen.  
✔ Er könnte schon zu Hause sein.  

Meaning:
Maybe yes, maybe no.

---

#### 🔹 Difference

| Indikativ | Konjunktiv II |
|------------|---------------|
| Ich kann kommen. | Ich könnte kommen. |
| (I am able to) | (Maybe I would / possibly) |

"könnte" = less certain / more polite

---

#### 🔹 With Nebensatz

Wir könnten zu Hause bleiben,  
weil es regnet.

Er könnte kommen,  
wenn er Zeit hat.

---

#### 🔹 Quick Memory

kann → real ability  
könnte → polite / possible / hypothetical  

Very important for B1 level speaking.

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 Konjunktiv II – könnte (können) Part II</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Formation

Konjunktiv II = Präteritum + Umlaut

konnte → könnte

---

### 🔹 Conjugation

| Pronoun | Form |
|----------|--------|
| ich | könnte |
| du | könntest |
| er/sie/es | könnte |
| wir | könnten |
| ihr | könntet |
| sie/Sie | könnten |

---

### 🔹 Sentence Structure

Subject + könnte + ... + Infinitive (END)

**Example:**
Du könntest mehr lernen.

---

</td>

<td valign="top">

### 🔹 Usage

### 1️⃣ Polite Requests

- Könnten Sie mir helfen?
- Könntest du bitte die Tür schließen?
- Könnten Sie bitte langsamer sprechen?

More polite than:
Kannst du...?

---

### 2️⃣ Suggestions / Advice

- Du könntest mehr Obst essen.
- Du könntest früher schlafen gehen.
- Wir könnten morgen ins Restaurant gehen.

---

### 3️⃣ Possibility (Not certain)

- Es könnte morgen regnen.
- Er könnte schon zu Hause sein.
- Der Zug könnte verspätet sein.

---

### 🔹 Comparison

| Form | Meaning |
|------|---------|
| kann | real ability |
| konnte | past ability |
| könnte | possible / polite / hypothetical |

---

### 🔹 Example Comparison

Mach die Tür zu! (Direct)

Kannst du die Tür schließen? (Normal)

Könntest du bitte die Tür schließen? (Very polite)

---

Very important for B1 speaking and writing.

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 Giving Advice: könnte vs sollte vs Imperativ</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 1️⃣ sollte (Konjunktiv II von sollen)

### Meaning:
Strong advice / recommendation  
More serious than "könnte"

### Structure:
Subject + solltest + ... + Infinitive

✔ Du solltest früher schlafen gehen.  
✔ Du solltest weniger Kaffee trinken.  
✔ Ihr solltet regelmäßig Pause machen.

Tone:
Advice with expectation.

---

### 🔹 2️⃣ könnte (Konjunktiv II von können)

### Meaning:
Soft suggestion  
Less strong than "sollte"

### Structure:
Subject + könntest + ... + Infinitive

✔ Du könntest mehr Wasser trinken.  
✔ Du könntest mit anderen im Kurs sprechen.  
✔ Wir könnten einen Spaziergang machen.

**Tone:**
Optional idea / possibility.

---

</td>

<td valign="top">

### 🔹 3️⃣ Imperativ

### Meaning:
Direct command or strong advice.

### Structure:

Verb (Position 1!)

✔ Geh früher schlafen!  
✔ Trink nicht so viel Kaffee!  
✔ Sprich mit anderen im Kurs!  
✔ Macht regelmäßig Pause!

Tone:
Direct / stronger / sometimes strict.

---

### 🔹 Comparison Table

| Form | Strength | Example |
|-------|----------|----------|
| könnte | Soft suggestion | Du könntest mehr lernen. |
| sollte | Strong advice | Du solltest mehr lernen. |
| Imperativ | Direct command | Lern mehr! |

---

### 🔹 Same Idea – 3 Versions

**Example:** Sleeping earlier

Soft:
Du könntest früher schlafen gehen.

Normal advice:
Du solltest früher schlafen gehen.

Direct:
Geh früher schlafen!

---

### 🔹 When to Use What?

Use könnte:
• When giving polite ideas  
• Friendly conversations  

Use sollte:
• When giving serious advice  
• Teacher → Student  
• Doctor → Patient  

Use Imperativ:
• Close friends  
• Instructions  
• Commands  

---

### 🔹 Exam Tip (B1)

In writing:
"sollte" is safest.

In speaking:
"könnte" sounds polite.

Imperativ can sound too strong if not careful.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Präpositionen – ohne (Akk) & mit (Dativ)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 1️⃣ ohne + Akkusativ

**Meaning:**
without

**Case:**
ALWAYS Akkusativ

---

### Article Forms (Akkusativ)

| Gender | Article |
|--------|----------|
| m | den / einen / keinen / meinen |
| f | die / eine / keine / meine |
| n | das / ein / kein / mein |
| pl | die / keine / meine |

---

### Examples

- Ohne Musik kann ich nicht leben.  
- Ohne Wasser überleben Pflanzen nicht.  
- Ich gehe ohne meinen Schlüssel.  
- Er kommt ohne seine Freunde.

</td>

<td valign="top">

### 🔹 2️⃣ mit + Dativ

**Meaning:**
with

**Case:**
ALWAYS Dativ

---

### Article Forms (Dativ)

| Gender | Article |
|--------|----------|
| m | dem / einem / keinem / meinem |
| f | der / einer / keiner / meiner |
| n | dem / einem / keinem / meinem |
| pl | den (+n) / keinen / meinen |

⚠ Plural Dativ → noun gets +n

---

### Examples

- Ich gehe mit meinem Freund spazieren.  
- Sie spricht mit ihrer Lehrerin.  
- Ich fahre mit dem Zug.  
- Ich treffe mich mit meinen Freunden.

---

### 🔹 Quick Memory

ohne → Akk  
mit → Dat  

Very important fixed-case prepositions.

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 Interrogativartikel – Was für … ?</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Meaning

"Was für … ?" means:

**What kind of / What type of**

It is used when we ask about:

• type  
• category  
• quality  
• character of a person or thing

**Example:**

Was für ein Buch ist das?  
(What kind of book is that?)

---

### 🔹 Basic Structure

Was für + Artikel + Nomen

**Example:**

Was für ein Film ist das?

---

### 🔹 Declension Table

| Case | Masculine | Neuter | Feminine | Plural |
|-----|-----------|--------|----------|--------|
| Nominativ | was für ein | was für ein | was für eine | was für |
| Akkusativ | was für einen | was für ein | was für eine | was für |
| Dativ | was für einem | was für einem | was für einer | was für |

---

### 🔹 Nominativ Examples

Was für ein Buch ist das?  
Was für eine Tasche ist das?  
Was für ein Mensch bist du?  
Was für Menschen sind das?

Meaning:

What kind of book is that?  
What kind of bag is that?

---

### 🔹 Akkusativ Examples

Was für einen Film hast du gesehen?  
Was für eine Tasche hast du gekauft?  
Was für ein Buch hast du gelesen?  
Was für Bücher liest du gern?

Explanation:
Here the noun is the **object of the sentence**, so we use **Akkusativ**.

---
### 🔹 Dativ Examples

Some verbs require **Dativ**.

Example verb:

helfen → always takes Dativ

Examples:

Was für einem Mann hast du geholfen?  
Was für einem Freund hast du geholfen?  
Was für Gruppen hast du geholfen?

Meaning:

What kind of man did you help?

</td>

<td valign="top">





### 🔹 Questions with Prepositions

Sometimes the question starts with a **preposition**.

Examples:

Mit was für einem Stift schreibst du?  
Bei was für einer Firma arbeitest du?  
Aus was für einem Land kommst du?  
Aus was für einem Material ist das?

Meaning:

What kind of pen do you write with?  
What kind of company do you work at?  
What kind of country do you come from?

---

### 🔹 Word Order Rule

If the sentence **starts with a preposition**, the **subject often comes after the verb**.

Structure:

Preposition + was für + Objekt + Verb + Subject

**Example:**

Mit was für einem Stift schreibst du?

Breakdown:

Mit = preposition  
was für einem Stift = object (Dativ)  
schreibst = verb  
du = subject

---

### 🔹 When a Modal Verb is Used

If the question contains a **modal verb**, the structure changes.

The **modal verb stays in position 2**, and the **main verb goes to the end (infinitive)**.

Structure:

Question word + Object + Modalverb + Subject + Main Verb

**Example:**

Was für ein Kleid soll ich zur Party anziehen?

Breakdown:

Was für ein Kleid = object  
soll = modal verb  
ich = subject  
anziehen = main verb (infinitive)

Meaning:

What kind of dress should I wear to the party?

---

### 🔹 More Modal Verb Examples

Was für ein Buch soll ich lesen?  
Was für einen Film können wir sehen?  
Mit was für einem Auto könnte er fahren?  
In was für einer Wohnung möchtest du wohnen?

---

### 🔹 "Was für" in Exclamations (Ausrufe)

"Was für" can also express **strong emotion or surprise**.

In this case the sentence is **not a question**, but an **exclamation**.

Structure:

Was für + Artikel + Nomen !

Meaning:

"What a …!"

Examples:

Was für ein Ausflug!  
What a trip!

Was für eine Hochzeit!  
What a wedding!

Was für ein Fest!  
What a festival!

Was für tolle Schuhe!  
What beautiful shoes!

Was für ein Tag!  
What a day!

Was für ein Glück!  
What luck!

Was für ein Pech!  
What bad luck!

---

### 🔹 Structure Difference

Question:

Was für ein Buch liest du?

**Exclamation:**

Was für ein Buch!

Important:

• Exclamations **do not need a verb**  
• They end with **! instead of ?**

**Example:**

Was für ein schönes Haus!  
What a beautiful house!

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Reflexive Verben – Reflexive Verbs</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Reflexive Verbs?  
(German: Reflexive Verben)

Reflexive verbs describe actions where **the subject and object are the same person**.

The action returns to the subject.

Example

Ich wasche mich.  
(I wash myself)

Subject → ich  
Object → mich  

Both refer to the **same person**.

---

### 🔹 Reflexive Pronouns  
(German: Reflexivpronomen)

| Person | Reflexivpronomen | English |
|------|------|------|
| ich | mich | myself |
| du | dich | yourself |
| er / sie / es | sich | himself / herself |
| wir | uns | ourselves |
| ihr | euch | yourselves |
| sie / Sie | sich | themselves / yourself |

⚠ Important

3rd person and formal **Sie** always use **sich**.

---

### 🔹 Example Sentences

- Ich erinnere mich an das Fest.  
- Wir treffen uns heute Abend.  
- Du konzentrierst dich auf die Aufgabe.  
- Er freut sich auf den Urlaub.

Meaning

The action goes **back to the subject**.

</td>

<td valign="top">

### 🔹 Echte reflexive Verben  
(True Reflexive Verbs)

These verbs **always require a reflexive pronoun**.

Without the pronoun the sentence becomes incorrect.

| Verb | Meaning |
|------|------|
| sich erholen | to recover |
| sich beeilen | to hurry |
| sich schämen | to be ashamed |

Examples

- Ich erhole mich schnell.  
- Wir müssen uns beeilen.  
- Ich schäme mich.

---

### 🔹 Unechte reflexive Verben  
(Optional Reflexive Verbs)

These verbs can be used **with or without reflexive pronouns**.

| Reflexive Sentence | Non-Reflexive Sentence |
|------|------|
| Ich erinnere mich an die Reise. | Ich erinnere meine Schwester. |
| Ich treffe mich mit Freunden. | Ich treffe meine Freunde. |
| Ich wasche mich. | Ich wasche mein Auto. |

Meaning changes depending on the object.

---

### 🔹 Word Order  
(German: Wortstellung)

If the sentence starts with the subject:

Ich fühle mich gut.

If the sentence starts with time/place:

Heute fühle ich mich gut.

Examples

- Heute fühle ich mich nicht gut.  
- Peter hat sich besser gefühlt.

Important rule

Reflexive pronouns usually appear  
**before other objects**.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Reflexive Verben – Akkusativ vs Dativ</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Reflexive Verben mit Akkusativ  
(Reflexive Verbs with Accusative)

Most reflexive verbs use **Akkusativ reflexive pronouns**.

Structure

Subject + Verb + Reflexivpronomen

Example

Ich wasche mich.

---

### 🔹 Akkusativ Reflexivpronomen

| Person | Akkusativ | English |
|------|------|------|
| ich | mich | myself |
| du | dich | yourself |
| er / sie / es | sich | himself |
| wir | uns | ourselves |
| ihr | euch | yourselves |
| sie / Sie | sich | themselves |

---

### 🔹 Example Sentences

- Ich erinnere mich an das Fest.  
- Wir treffen uns heute Abend.  
- Du solltest dich konzentrieren.  
- Er freut sich auf den Urlaub.

**Meaning:**

The action affects the **subject directly**.

</td>

<td valign="top">

### 🔹 Reflexive Verben mit Dativ  
(Reflexive Verbs with Dative)

Dative appears when the sentence already has  
**an Akkusativ object**.

Structure

Subject + Verb + Dativ Reflexiv + Akkusativ Objekt

Example

Ich wasche **mir** die Hände.

---

### 🔹 Dativ Reflexivpronomen

| Person | Dativ | English |
|------|------|------|
| ich | mir | to myself |
| du | dir | to yourself |
| er / sie / es | sich | to himself |
| wir | uns | to ourselves |
| ihr | euch | to yourselves |
| sie / Sie | sich | to themselves |

---

### 🔹 Akkusativ vs Dativ

| Sentence | Explanation |
|------|------|
| Ich wasche mich. | No object → Akkusativ |
| Ich wasche mir die Hände. | Object present → Dativ |
| Ich kämme mich. | Akkusativ |
| Ich kämme mir die Haare. | Dativ |

---

### 🔹 Common Body Actions  
(Körperhandlungen)

| Verb | Example |
|------|------|
| sich die Hände waschen | Ich wasche mir die Hände |
| sich die Zähne putzen | Ich putze mir die Zähne |
| sich die Haare kämmen | Ich kämme mir die Haare |
| sich das Gesicht waschen | Ich wasche mir das Gesicht |
| sich die Jacke anziehen | Ich ziehe mir die Jacke an |

---

### 🔹 Quick Rule  
(Wichtige Regel)

No object → **Akkusativ**

Ich wasche mich.

Object present → **Dativ**

Ich wasche mir die Hände.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Komparativ und Superlativ – Comparative and Superlative</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Grundidee – Basic Idea

In German adjectives have **three forms**.

German terms:

| Form | German | English |
|-----|------|------|
| Grundform | Positiv | basic form |
| Komparativ | Comparative | more |
| Superlativ | Superlative | most |

Example

| Form | Example |
|----|----|
| Positiv | schön |
| Komparativ | schöner |
| Superlativ | am schönsten |

Meaning

schön → beautiful  
schöner → more beautiful  
am schönsten → most beautiful

---

### 🔹 Komparativ bilden – Forming the Comparative

Rule

Komparativ = **Adjektiv + er**

Examples

| Adjektiv | Komparativ |
|------|------|
| bekannt | bekannter |
| wild | wilder |
| nass | nasser |
| heiß | heißer |
| hübsch | hübscher |
| stolz | stolzer |

Example sentence

Ali ist größer als Jonas.  
(Ali is taller than Jonas)

Important word

**als = than**

---

### 🔹 Superlativ bilden – Forming the Superlative

Rule

Superlativ = **am + Adjektiv + sten**

Example

schön → am schönsten

Examples

| Adjektiv | Superlativ |
|------|------|
| bekannt | am bekanntesten |
| wild | am wildesten |
| nass | am nassesten |
| heiß | am heißesten |
| hübsch | am hübschesten |

Example sentence

Ali ist am größten.

(Ali is the tallest)

---

### 🔹 Important Adjective Forms

| Grundform | Komparativ | Superlativ |
|------|------|------|
| schön | schöner | am schönsten |
| teuer | teurer | am teuersten |
| billig | billiger | am billigsten |
| praktisch | praktischer | am praktischsten |
| alt | älter | am ältesten |
| neu | neuer | am neuesten |
| heiß | heißer | am heißesten |
| kalt | kälter | am kältesten |
| viel | mehr | am meisten |
| oft / häufig | öfter / häufiger | am häufigsten |
| gut | besser | am besten |
| gern | lieber | am liebsten |
| hoch | höher | am höchsten |
| nah | näher | am nächsten |



</td>

<td valign="top">

### 🔹 Special Ending Rule

Adjectives ending with

t, d, s, ß, sch, z

take **-esten** in superlative.

Examples

| Adjektiv | Superlativ |
|------|------|
| heiß | am heißesten |
| stolz | am stolzesten |
| hübsch | am hübschesten |

---

### 🔹 Umlaut Rule (One-syllable adjectives)

Many **short adjectives** add an umlaut.

**Rule**

a → ä  
o → ö  
u → ü

**Examples**

| Positiv | Komparativ | Superlativ |
|------|------|------|
| alt | älter | am ältesten |
| groß | größer | am größten |
| klug | klüger | am klügsten |

⚠ Important

Not all adjectives take umlaut.

---

### 🔹 Exceptions (kein Umlaut)

| Positiv | Komparativ | Superlativ |
|------|------|------|
| schlau | schlauer | am schlauesten |
| rau | rauer | am rauesten |
| froh | froher | am frohsten |
| klar | klarer | am klarsten |
| voll | voller | am vollsten |

---

### 🔹 Special Rule (-el / -er adjectives)

Adjectives ending with **-el** or **-er** lose the letter **e** in comparative.

Examples

| Positiv | Komparativ | Superlativ |
|------|------|------|
| teuer | teurer | am teuersten |
| dunkel | dunkler | am dunkelsten |



---

### 🔹 Example Sentences

1️⃣ Handy ist gut, Tablet ist besser, aber Laptop ist am besten.

2️⃣ K2 ist höher, aber der Mount Everest ist am höchsten.

3️⃣ Burj Khalifa ist am größten.

4️⃣ Sie ist am jüngsten in der Familie.

5️⃣ Das iPhone ist am besten.

6️⃣ ChatGPT ist am praktischsten.

7️⃣ Mein Handy ist am ältesten.

8️⃣ Dieser Stuhl ist schöner als dieser, aber dieser ist am schönsten.

9️⃣ Das Buch ist am interessantesten.

10️⃣ Heute bin ich am stolzesten, weil mein Mann sehr erfolgreich ist.

11️⃣ Mein Vater ist am stolzesten, weil seine Tochter Ärztin geworden ist.

12️⃣ Vorgestern war heißer als gestern, aber heute ist am heißesten.

</td>
</tr>


</table>



</details>
<details>
<summary><b>📌 Vergleichssätze – Comparisons (als / wie)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Basic Idea

German uses **two words for comparison**:

| Word | Meaning | Usage |
|-----|-----|-----|
| wie | as | when things are equal |
| als | than | when things are different |

Example

- Mein Handy ist **so teuer wie** dein Handy.  
- Mein Handy ist **teurer als** dein Handy.

---

### 🔹 Gleichheit (Equality) → wie

If **two things are equal**, we use:

Structure

**so / genauso + Adjektiv + wie**

**Example:**

| Sentence | Meaning |
|-----|-----|
| Sven ist so groß wie sein Bruder. | Sven is as tall as his brother |
| Mein Handy ist genauso teuer wie dein Handy. | My phone is just as expensive as yours |
| Dieses Rätsel ist genauso schwierig wie das letzte. | This puzzle is as difficult as the last one |

---

### 🔹 Negative Equality

Structure

**nicht so + Adjektiv + wie**

**Example:**

- Die Preise sind **nicht so hoch wie** früher.  
- Sven ist **nicht so groß wie** sein Bruder.  
- Mein Handy ist **nicht so wichtig wie** mein Laptop.

---

### 🔹 Important Words

| Word | Meaning |
|-----|-----|
| so | as |
| genauso | exactly / just as |
| nicht so | not as |

</td>

<td valign="top">

### 🔹 Unterschied (Difference) → als

If **there is a difference**, we use:

Structure

**Komparativ + als**

Example

| Sentence | Meaning |
|-----|-----|
| Sven ist größer als sein Bruder. | Sven is taller than his brother |
| Mein Handy ist teurer als dein Handy. | My phone is more expensive than yours |
| In Berlin ist es kälter als in Darmstadt. | Berlin is colder than Darmstadt |

---

### 🔹 More Examples

- Dieses Rätsel ist **schwieriger als** das letzte.  
- Heute bin ich **müder als** gestern.  
- Sie spricht **besser Deutsch als** ich.  
- Du bist **später gekommen als** gestern.  

---

### 🔹 Comparison with Cities / Places

- Karachi ist **größer als** Lahore.  
- Lahore ist **nicht so groß wie** Karachi.  
- In Lahore ist es **heißer als** in Pindi.

---

### 🔹 Comparison with Time

- Heute hat es **stärker geregnet als** gestern.  
- Heute fühle ich mich **genauso müde wie** gestern.

---

### 🔹 Quick Memory

Equality  
➡ **so / genauso + Adjektiv + wie**

Difference  
➡ **Komparativ + als**

Example

- Mein Laptop ist **teurer als** mein Handy.  
- Mein Fernseher ist **genauso wichtig wie** mein Computer.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Dativ Präpositionen – Dative Prepositions</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Dativ Präpositionen?

Some German prepositions **always require the Dativ case**.

These are called **Dativ Präpositionen**.

Common ones:

| Preposition | Meaning |
|------|------|
| zu | to |
| nach | to / after |
| bei | at / with |
| mit | with |
| aus | from / out of |
| von | from / of |

Example

- Ich fahre **mit dem Bus**.  
- Ich bin **bei meinem Freund**.

---

### 🔹 Contractions (Important)

Some prepositions combine with **dem** or **der**.

| Combination | Short Form |
|------|------|
| zu + dem | zum |
| zu + der | zur |
| bei + dem | beim |
| von + dem | vom |

Examples

- Ich gehe **zum Arzt**.  
- Ich gehe **zur Schule**.  
- Ich bin **beim Arzt**.  
- Ich komme **vom Arzt**.

---

### 🔹 zu

Meaning: **to / towards**

Used with **people, places, and destinations**.

Examples

- Ich gehe **zum Arzt**.  
- Wir fahren **zur Schule**.  
- Ich gehe **zu meinem Freund**.

</td>

<td valign="top">

### 🔹 nach

Meaning: **to / after**

Used mainly with:

• cities  
• countries (without article)  
• directions  

Examples

- Ich fahre **nach Berlin**.  
- Ich fliege **nach Pakistan**.  
- Wir gehen **nach Hause**.

---

### 🔹 bei

Meaning: **at / with**

Examples

- Ich bin **bei meinen Eltern**.  
- Ich arbeite **bei Siemens**.  
- Ich bin **bei Ali**.

---

### 🔹 mit

Meaning: **with**

Examples

- Ich fahre **mit dem Bus**.  
- Ich gehe **mit meinem Freund** ins Kino.  
- Sie spricht **mit ihrer Lehrerin**.

---

### 🔹 aus

Meaning: **from / out of**

Examples

- Ich komme **aus Pakistan**.  
- Der Tisch ist **aus Holz**.  
- Dieser Stuhl ist **aus Metall**.

---

### 🔹 von

Meaning: **from / of**

Examples

- Ich komme **vom Arzt**.  
- Das Geschenk ist **von meinem Vater**.  
- Frankfurt liegt **nördlich von Darmstadt**.  
- Er ist einer **von meinen Freunden**.

---

### 🔹 Quick Memory

All these prepositions use **Dativ**:

zu  
nach  
bei  
mit  
aus  
von  

Example pattern

Preposition + **Dativ Artikel**

- mit **dem** Bus  
- bei **meinem** Freund  
- von **meinem** Vater  

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Trennbare und Untrennbare Verben – Separable vs Inseparable Verbs</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 1️⃣ Trennbare Verben (Separable Verbs)

Some German verbs have a **prefix (Vorsilbe)** that separates in the sentence.

Example verb

einladen = ein + laden

Example sentence (Präsens)

Ich **lade** meine Freunde **ein**.

Structure

Subject + Verb (Position 2) + ... + Prefix (END)

---

### 🔹 Perfekt Rule (Very Important)

For separable verbs in **Perfekt**:

✔ **ge** appears **between prefix and verb**

Pattern

Prefix + **ge** + Verb

Examples

| Verb | Perfekt |
|------|------|
| einladen | hat eingeladen |
| anrufen | hat angerufen |
| abholen | hat abgeholt |
| aussprechen | hat ausgesprochen |
| anbieten | hat angeboten |
| aussehen | hat ausgesehen |
| ankommen | ist angekommen |

---

### 🔹 Sentence Structure (Perfekt)

In Perfekt:

- **haben / sein → position 2**
- **Partizip II → sentence end**

Example

Ich **habe** meine Freunde **eingeladen**.

---

### 🔹 Examples

Präsens

Ich lade alle meine Freunde zur Party **ein**.

Perfekt

Ich habe alle meine Freunde zur Party **eingeladen**.

---

More examples

- Gestern habe ich meine Mutter **angerufen**.  
- Ich habe meine Freunde **eingeladen**.  
- Heute ist mein Paket **angekommen**.  
- Der Zug ist zu spät **angekommen**.

---

### 🔹 More Separable Verbs

| Verb | Perfekt |
|------|------|
| mithelfen | hat mitgeholfen |
| sprechen | gesprochen |
| helfen | geholfen |
| kommen | gekommen |
| aussprechen | ausgesprochen |

Example

Alle meine Freunde sind zur Party **gekommen**.

---

### 🔹 2️⃣ Untrennbare Verben (Inseparable Verbs)

Some verbs have prefixes that **never separate**.

These prefixes stay **attached to the verb**.

Important prefixes

| Prefix |
|------|
| be- |
| er- |
| ge- |
| emp- |
| ent- |
| ver- |
| zer- |
| miss- |
| über- |
| miß- |

Example verbs

| Verb | Meaning |
|------|------|
| bekommen | to receive |
| besuchen | to visit |
| ergänzen | to complete |
| erreichen | to reach |
| empfehlen | to recommend |
| entkommen | to escape |
| verkaufen | to sell |
| verbinden | to connect |
| zerbrechen | to break |
| übernachten | to stay overnight |



</td>

<td valign="top">

### 🔹 Perfekt Examples

| Verb | Perfekt |
|------|------|
| bekommen | hat bekommen |
| ergänzen | hat ergänzt |
| gefallen | hat gefallen |
| empfehlen | hat empfohlen |
| entkommen | ist entkommen |
| verkaufen | hat verkauft |
| übernachten | hat übernachtet |
| verbinden | hat verbunden |

---

### 🔹 Sentence Structure

Präsens

Ich **bekomme** einen Brief.

Perfekt

Ich **habe** einen Brief **bekommen**.

Pattern

Subject + haben/sein + ... + Partizip II

---

### 🔹 Important Difference

| Type | ge in Perfekt? |
|------|------|
| Trennbare Verben | YES |
| Untrennbare Verben | NO |

Example

einladen → eingeladen  
bekommen → bekommen

---

### 🔹 Perfekt Sentence Structure

German **Perfekt tense** structure

Position 1 | Position 2 | Middle | End
---|---|---|---
Subject | haben / sein | object / time | Partizip II

Example

Ich | habe | einen Brief | bekommen.

---

### 🔹 Another Example

Ich | habe | meine Freunde | eingeladen.

Example with **sein**

Alle meine Freunde | sind | zur Party | gekommen.

---

### 🔹 haben vs sein

Use **haben** with most verbs.

Use **sein** with:

• movement  
• change of state

Example

Ich habe meine Mutter angerufen.  
Der Zug ist angekommen.

---

### 🔹 Example Text (From Exercise)

Letztes Jahr habe ich in Heidelberg **studiert**.

Im Sommer bin ich mit drei Freunden im Auto nach München **gefahren**.

Unterwegs ist etwas **explodiert**.

Der Reifen war kaputt!

Wir haben **diskutiert**, was wir jetzt machen.

Ein Freund hat dann **probiert**, den Reifen zu wechseln.

Und er hat wirklich das Auto **repariert**.

Wir haben ihm **gratuliert** und sind gut in München **angekommen**.

---

### 🔹 What You Learn From This Text

✔ Perfekt uses **haben / sein**  
✔ Partizip II is at the **end**  
✔ Some verbs take **sein** (fahren, kommen)  
✔ Most verbs take **haben**

**Example pattern**

Ich **habe studiert**.  
Wir **haben diskutiert**.  
Ich **bin gefahren**.  
Wir **sind angekommen**.

---

### 🔹 Important Rule

For **untrennbare Verben**:

❌ No **ge-** in Perfekt

Example

bekommen → hat bekommen  
empfehlen → hat empfohlen  
verkaufen → hat verkauft

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Wechselpräpositionen – an / auf (Location) Part I</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 "an" + Dativ (Location)

Used for **edges, borders, vertical surfaces, stations, furniture**.

| Category | Example |
|---|---|
| Wasser | am See |
| Wasser | am Meer |
| Wasser | an der See |
| Wasser | am Strand |
| Wasser | am Fluss |
| Wasser | am Ufer |
| Wasser | am Bach |
| Wasser | an der Küste |
| Vertical Surface | an der Wand |
| Vertical Surface | am Fenster |
| Vertical Surface | an der Tür |
| Vertical Surface | an der Mauer |
| Vertical Surface | an der Tafel |
| Vertical Surface | an der Grenze |
| Station / Place | an der Ampel |
| Station / Place | an der Haltestelle |
| Station / Place | an der Ecke |
| Station / Place | an der Kreuzung |
| Station / Place | an der Kasse |
| Station / Place | in der Schlange an der Kasse |
| Station / Place | am Bahnhof |
| Working Furniture | am Tisch |
| Working Furniture | am Schreibtisch |
| Working Furniture | am Computer |
| Working Furniture | am Waschbecken |
| Working Furniture | am Herd |

**Examples**

- Ich sitze **am Tisch**.  
- Er arbeitet **am Computer**.  
- Wir warten **an der Haltestelle**.  
- Das Bild hängt **an der Wand**.

---

### 🔹 Meaning of "an"

"an" usually means:

✔ at  
✔ on (vertical surface)  
✔ near edge/border

Examples:

- **am Meer** → at the sea  
- **an der Wand** → on the wall  
- **am Computer** → at the computer

</td>

<td valign="top">

### 🔹 "auf" + Dativ (Location)

Used for **flat surfaces, open areas, platforms**.

| Place | Example |
|---|---|
| Mountain | auf dem Berg |
| Tower | auf dem Turm |
| Hill | auf dem Hügel |
| Balcony | auf dem Balkon |
| Island | auf der Insel |

**Examples**

- Wir stehen **auf dem Balkon**.  
- Das Schloss liegt **auf dem Hügel**.  
- Er lebt **auf der Insel**.  
- Die Burg steht **auf dem Berg**.

---

### 🔹 Meaning of "auf"

"auf" usually means:

✔ on top of  
✔ on a surface  
✔ on open area

Examples:

- **auf dem Tisch** → on the table  
- **auf dem Berg** → on the mountain  
- **auf der Insel** → on the island

---

### 🔹 Case Rule (VERY IMPORTANT)

Both **an** and **auf** are **Wechselpräpositionen**.

| Situation | Case |
|---|---|
| Location (Wo?) | **Dativ** |
| Movement (Wohin?) | **Akkusativ** |

Examples:

Wo?  
Ich sitze **am Tisch**. (Dativ)

Wohin?  
Ich setze mich **an den Tisch**. (Akkusativ)

Wo?  
Er steht **auf dem Balkon**.

Wohin?  
Er geht **auf den Balkon**.

---

### 🔹 Memory Trick

Use **AN** when something is:

✔ touching a **vertical surface**  
✔ at an **edge or border**  
✔ at a **station / functional place**

Use **AUF** when something is:

✔ **on top of a surface**  
✔ on a **mountain / hill / island**

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Wechselpräpositionen mit Dativ und Akkusativ (Two-Way Prepositions) Part II</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Wechselpräpositionen?

**Wechselpräpositionen** are prepositions that can take **two cases**:

• **Dativ**  
• **Akkusativ**

The case depends on **movement or position**.

| Question | Case | Meaning |
|---|---|---|
| Wo? | Dativ | location / position |
| Wohin? | Akkusativ | direction / movement |

Example

Ich bin **im Kino**.  
(I am in the cinema – location)

Ich gehe **ins Kino**.  
(I go into the cinema – movement)

---

### 🔹 The 9 Wechselpräpositionen

| Preposition | Meaning |
|---|---|
| an | on / at |
| auf | on |
| in | in / into |
| über | over |
| unter | under |
| vor | in front of |
| hinter | behind |
| neben | next to |
| zwischen | between |

---

### 🔹 Wo? → Dativ (Location)

Used when something **is already in a place**.

Structure

Verb + Preposition + **Dativ**

Examples

Der Kuchen steht **auf dem Stuhl**.  
(The cake is on the chair)

Die Vase steht **auf dem Herd**.  
(The vase is on the stove)

Die Socken liegen **auf dem Tisch**.  
(The socks lie on the table)

Die Decke liegt **unter dem Tisch**.  
(The blanket lies under the table)

Das Papier liegt **auf dem Boden**.  
(The paper lies on the floor)

</td>

<td valign="top">

### 🔹 Wohin? → Akkusativ (Direction)

Used when there is **movement toward a place**.

Structure

Verb + Preposition + **Akkusativ**

Examples

Sie legt die Decke **auf den Tisch**.  
(She puts the blanket onto the table)

Sie legt die Socken **in den Schrank**.  
(She puts the socks into the cupboard)

Sie legt das Papier **ins Regal**.  
(She puts the paper into the shelf)

Sie stellt den Kuchen **auf den Tisch**.  
(She places the cake onto the table)

Sie stellt die Vase **neben das Sofa**.  
(She puts the vase next to the sofa)

---

### 🔹 Example with "Kino"

Wo? (location)

Ich bin **im Kino**.

Wohin? (direction)

Ich gehe **ins Kino**.

Explanation

in + dem = **im**  
in + das = **ins**

---

### 🔹 More Examples

Wo?

Wir studieren **in München**.  
Die Kinder spielen **auf dem Spielplatz**.

Wohin?

Wir gehen **in die Mediathek**.  
Sie bringt den Müll **in die Mülltonne**.

---

### 🔹 Typical Verbs

Verbs that answer **Wohin? (movement)**

| Verb | Meaning |
|---|---|
| gehen | to go |
| bringen | to bring |
| fahren | to drive |
| steigen | to climb |
| fliegen | to fly |

Example

Der Vogel fliegt **über den Baum**.

---

Verbs that answer **Wo? (location)**

| Verb | Meaning |
|---|---|
| sein | to be |
| sitzen | to sit |
| stehen | to stand |
| liegen | to lie |
| bleiben | to stay |

Example

Der Kuchen steht **auf dem Tisch**.

---

### 🔹 Contractions

| Preposition + Article | Short form |
|---|---|
| in dem | im |
| in das | ins |
| an dem | am |
| an das | ans |

Examples

Ich bin **im** Kino.  
Ich gehe **ins** Kino.

---

### 🔹 Quick Memory Trick

Ask yourself one question:

**Movement? → Akkusativ**

Example

Ich lege das Buch **auf den Tisch**.

**Position? → Dativ**

Example

Das Buch liegt **auf dem Tisch**.

⚡ Easy rule

**Wohin? → Akkusativ**  
**Wo? → Dativ**

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 deshalb vs trotzdem – Sentence Connectors (Result & Contrast)</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 deshalb

Meaning:

**therefore / that's why / for this reason**

Used to express a **result or consequence** of a previous sentence.

---

### 🔹 Structure

deshalb can appear:

| Position | Example |
|------|------|
| Position 1 | Deshalb nehme ich einen Regenschirm mit. |
| Middle of sentence | Ich nehme deshalb einen Regenschirm mit. |

✔ Verb must stay in **position 2**

---

### 🔹 Examples

- Es regnet stark, **deshalb nehme ich einen Regenschirm mit.**  
- Ich habe gut gelernt, **deshalb habe ich die Prüfung bestanden.**  
- Er ist krank, **deshalb bleibt er zu Hause.**  
- Wir haben keinen Zucker mehr, **deshalb können wir keinen Kuchen backen.**  
- Sie arbeitet viel, **deshalb ist sie oft müde.**

---

### 🔹 Similar meaning with "weil"

Es regnet stark, **deshalb nehme ich einen Regenschirm mit.**

Same meaning:

Ich nehme einen Regenschirm mit, **weil es stark regnet.**

</td>

<td valign="top">

### 🔹 trotzdem

Meaning:

**nevertheless / despite that / still**

Used when something happens **even though there is a reason against it**.

---

### 🔹 Structure

| Pattern | Example |
|------|------|
| trotzdem + verb | Trotzdem gehe ich spazieren. |
| middle position | Ich gehe trotzdem spazieren. |

✔ Verb stays in **position 2**

---

### 🔹 Examples

- Es regnet stark, **trotzdem gehe ich spazieren.**  
- Ich war müde, **trotzdem habe ich weiter gelernt.**  
- Er ist krank, **trotzdem geht er zur Arbeit.**

---

### 🔹 trotzdem vs deshalb

| Word | Meaning |
|------|------|
| deshalb | result / consequence |
| trotzdem | contrast / unexpected result |

Example:

- Es regnet stark, **deshalb bleibe ich zu Hause.**  
- Es regnet stark, **trotzdem gehe ich spazieren.**

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 weil vs deshalb – Reason vs Result</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 weil

Meaning:

**because**

✔ Used to express a **reason (Grund)**  
✔ Introduces a **Nebensatz (subordinate clause)**  
✔ **Verb goes to the END**

---

### 🔹 Structure

Main clause + , + **weil + Subject + ... + Verb**

Example:

Ich nehme einen Regenschirm mit,  
**weil es stark regnet.**

---

### 🔹 Examples

- Ich bleibe zu Hause, **weil ich krank bin.**  
- Ich lerne Deutsch, **weil ich in Deutschland arbeiten möchte.**  
- Er geht früh schlafen, **weil er morgen arbeiten muss.**

---

### 🔹 Sentence Structure

| Part | Example |
|-----|-----|
| Main clause | Ich nehme einen Regenschirm mit |
| Subordinate clause | weil es stark regnet |

Verb in **weil-Satz goes to the end**

</td>

<td valign="top">

### 🔹 deshalb

Meaning:

**therefore / that's why**

✔ Shows **result or consequence (Folge)**  
✔ Used in a **Hauptsatz (main clause)**  
✔ Verb stays in **Position 2**

---

### 🔹 Sentence Field Structure

| 1. Hauptsatz | Position 1 | Position 2 | Mittelfeld | Satzende |
|---|---|---|---|---|
| Es regnet stark | deshalb | nehme | ich den Regenschirm | mit |

---

### 🔹 Alternative Structure

| 1. Hauptsatz | Position 1 | Position 2 | Mittelfeld | Satzende |
|---|---|---|---|---|
| Es regnet stark | ich | nehme | deshalb den Regenschirm | mit |

---

### 🔹 Example Sentences

- Es regnet stark, **deshalb nehme ich einen Regenschirm mit.**  
- Ich habe gut gelernt, **deshalb habe ich die Prüfung bestanden.**  
- Er ist krank, **deshalb bleibt er zu Hause.**

---

### 🔹 Quick Comparison

| Word | Meaning | Grammar |
|------|------|------|
| weil | because | Nebensatz (verb at end) |
| deshalb | therefore | Hauptsatz (verb position 2) |

Example:

Ich nehme einen Regenschirm mit, **weil es stark regnet.**  
Es regnet stark, **deshalb nehme ich einen Regenschirm mit.**

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 Verben mit Dativ (Dative Verbs) Part I</b> <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Dative Verbs?

Some German verbs **always take the Dative case (Dativ)**.

That means the **person affected by the action is in Dativ**, not Akkusativ.

Structure:

Subject + Verb + **Dative Object**

Example

- Ich helfe **dir**.  
(I help **you**)

---

### 🔹 Dativ Pronouns

| Nominativ | Dativ |
|-----------|-------|
| ich | mir |
| du | dir |
| er | ihm |
| sie | ihr |
| es | ihm |
| wir | uns |
| ihr | euch |
| Sie | Ihnen |
| sie | ihnen |

---

### 🔹 Abbreviations in Dictionaries

| Abbreviation | Meaning |
|-------------|--------|
| jdm. | jemandem (someone – Dativ) |
| jdn. | jemanden (someone – Akkusativ) |
| etw. | etwas (something) |

Example

- jdm helfen → to help someone


---

### 🔹 Common Dative Verbs

| Verb | Meaning |
|------|--------|
| antworten (jdm.) | to answer someone |
| folgen (jdm./etw.) | to follow |
| gefallen (jdm.) | to like |
| gehören (jdm.) | to belong |
| glauben (jdm.) | to believe |
| helfen (jdm.) | to help |
| passieren (jdm.) | to happen to someone |
| raten (jdm.) | to advise |
| schmecken (jdm.) | to taste |
| wehtun (jdm.) | to hurt |
| zuhören (jdm.) | to listen to |
| befehlen (jdm.) | to command |
| danken (jdm.) | to thank |
| fehlen (jdm./etw.) | to miss / lack |
| nachlaufen (jdm./etw.) | to run after |
| nachrennen (jdm./etw.) | to run after |
| passen (jdm.) | to suit |
| vertrauen (jdm.) | to trust |
| vergeben (jdm.) | to forgive |
| verzeihen (jdm.) | to forgive |



</td>

<td valign="top">

---

### 🔹 Example Sentences (from your picture)

antworten  
- Ich antworte **dir** später.  
(I will answer **you** later.)

folgen  
- Ich folge **dem Mann** unauffällig.  
(I follow **the man** unnoticed.)

gefallen  
- Gefällt **dir** deine neue Wohnung?  
(Do **you** like your new apartment?)  

- Die Wohnung gefällt **mir**.  
(I like the apartment.)

gehören  
- Das Auto gehört **dem neuen Nachbarn**.  
(The car belongs to **the new neighbour**.)

glauben  
- Warum glaubst du **mir** nicht?  
(Why don't you believe **me**?)

helfen  
- Sie hilft **dem alten Mann**.  
(She helps **the old man**.)

passieren  
- Passiert **dir** das öfter?  
(Does that happen to **you** often?)

raten  
- Der Arzt hat **meinem Opa** geraten, sich auszuruhen.  
(The doctor advised **my grandfather** to rest.)

schmecken  
- Pizza schmeckt **meiner Mutter** nicht.  
(Pizza doesn't taste good to **my mother**.)

wehtun  
- Sie hat **ihm** wehgetan.  
(She hurt **him**.)

zuhören  
- Die Schüler hören **dem Lehrer** zu.  
(The students listen to **the teacher**.)

befehlen  
- Der General befiehlt **dem Soldaten**, zu schießen.  
(The general orders **the soldier** to shoot.)

danken  
- Ich danke **dir** für deine Hilfe.  
(I thank **you** for your help.)

fehlen  
- Mir fehlen die Worte.  
(I have no words.)  

- Du fehlst **mir**.  
(I miss **you**.)

nachlaufen  
- Ich laufe **dem Ball** nach.  
(I run after **the ball**.)

nachrennen  
- Ich renne **dem Auto** nach.  
(I run after **the car**.)

passen  
- Die Hose passt **mir** nicht mehr.  
(The trousers no longer fit **me**.)

vertrauen  
- Ich vertraue **meinem Bruder**.  
(I trust **my brother**.)

vergeben  
- Ich kann **meinem Mann** nicht vergeben.  
(I cannot forgive **my husband**.)

verzeihen  
- Ich kann **meinem Mann** nicht verzeihen.  
(I cannot forgive **my husband**.)

---

### 🔹 Quick Memory

Common **Dative verbs**

- helfen  
- danken  
- gefallen  
- gehören  
- glauben  
- folgen  
- zuhören  
- vertrauen  

⚡ Easy trick

If the verb expresses **feeling / relation to someone**, it often uses **Dativ**.

Example

- gefallen **mir**  
- helfen **dir**  
- danken **ihm**

</td>
</tr>
</table>

</details>


<details>
<summary><b>📌 Verben mit Dativ (Dative Verbs) – Part II</b> <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Dative Verbs?

Some German verbs **require the Dative case (Dativ)**.

The **person receiving the action is in Dativ**.

Structure

Subject + Verb + **Dativ object**

Example

Ich widerspreche **dir**.  
(I contradict **you**.)

---

### 🔹 Abbreviations used

| Abbreviation | Meaning |
|--------------|--------|
| jdm. | jemandem (someone – Dativ) |
| jdn. | jemanden (someone – Akkusativ) |
| etw. | etwas (something) |

Example

jdm widersprechen → to contradict someone

---

### 🔹 Dativ Verbs (Part II)

| Verb | Meaning |
|------|--------|
| widersprechen (jdm.) | to contradict |
| zusehen (jdm. bei etw.) | to watch someone |
| fremdgehen (jdm.) | to cheat on someone |
| zustimmen (jdm./etw.) | to agree with |
| ähneln (jdm./etw.) | to resemble |
| begegnen (jdm.) | to meet / come across |
| beistehen (jdm.) | to stand by someone |
| beitreten (jdm./etw.) | to join |
| drohen (jdm. mit etw.) | to threaten |
| entgegengehen (jdm.) | to go to meet |
| entgegenfahren (jdm./etw.) | to drive towards |
| entgegenkommen (jdm.) | to come towards |
| gratulieren (jdm. zu etw.) | to congratulate |
| einfallen (jdm.) | to come to someone's mind |
| gehorchen (jdm./etw.) | to obey |
| genügen (jdm.) | to be enough |
| guttun (jdm./etw.) | to benefit |
| nützen (jdm./etw.) | to benefit |
| ausweichen (jdm./etw.) | to avoid |
| dienen (jdm.) | to serve |
| gelingen (jdm.) | to succeed |
| misslingen (jdm.) | to fail |
| sich nähern (jdm.) | to approach |
| sich zuwenden (jdm.) | to turn to someone |


---

### 🔹 Quick Memory Trick

Many **Dative verbs describe interaction with a person**.

Examples

- widersprechen **jemandem**  
- zustimmen **jemandem**  
- helfen **jemandem**  
- gratulieren **jemandem**

⚡ Easy rule

If the action **goes toward a person**, German often uses **Dativ**.

Example

Ich gratuliere **dir**.  
Ich widerspreche **dir**.
</td>

<td valign="top">

### 🔹 Example Sentences

widersprechen  
Der Chef widerspricht **seinem Mitarbeiter**.  
(The boss contradicts **his employee**.)

zusehen  
Kann ich **dir** bei deiner Arbeit zusehen?  
(Can I watch **you** while you work?)

fremdgehen  
Bist du **mir** fremdgegangen?  
(Have you cheated on **me**?)

zustimmen  
Der Politiker stimmt **dem neuen Gesetz** zu.  
(The politician agrees with **the new law**.)

ähneln  
Ich ähnele **meinem Bruder**.  
(I resemble **my brother**.)

begegnen  
Ich bin heute Morgen zufällig **einem alten Freund** begegnet.  
(I happened to meet **an old friend** this morning.)

beistehen  
Ich stehe **dir** in dieser schweren Zeit bei.  
(I stand by **you** in this difficult time.)

beitreten  
Ich bin gestern **einem Fußballclub** beigetreten.  
(I joined **a football club** yesterday.)

drohen  
Der Lehrer droht **den Schülern** mit extra Hausaufgaben.  
(The teacher threatens **the students** with extra homework.)

entgegengehen  
Ich gehe **dir** schon mal entgegen.  
(I go to meet **you**.)

entgegenfahren  
Er fährt **dir** schon entgegen.  
(He drives towards **you**.)

entgegenkommen  
Kommst du **mir** entgegen?  
(Are you coming towards **me**?)

gratulieren  
Ich gratuliere **dir** zum Geburtstag.  
(I congratulate **you** on your birthday.)

einfallen  
Fällt **dir** noch etwas ein?  
(Does anything else come to **your** mind?)

gehorchen  
Der Soldat gehorcht **dem General**.  
(The soldier obeys **the general**.)

genügen  
Das genügt **mir**.  
(That is enough for **me**.)

guttun  
Ein Urlaub würde **dir** guttun.  
(A vacation would do **you** good.)

nützen  
Der Sieg nützt **dem Team** nichts mehr.  
(The victory no longer benefits **the team**.)

ausweichen  
Du konntest **dem Hindernis** ausweichen.  
(You managed to avoid **the obstacle**.)

dienen  
Wie kann ich **Ihnen** dienen?  
(How can I serve **you**?)

gelingen  
Die Präsentation ist **ihm** gut gelungen.  
(The presentation succeeded **for him**.)

misslingen  
Der Test ist **mir** total misslungen.  
(The test completely failed **for me**.)

sich nähern  
Der Löwe nähert sich **seiner Beute**.  
(The lion approaches **its prey**.)

sich zuwenden  
Sie wendet sich immer **ihrem Chef** zu.  
(She always turns to **her boss**.)



</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Verbtypen – Transitive, Intransitive und Ditransitive Verben</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Intransitive Verben

**Definition**

Verben, die **kein Akkusativobjekt** brauchen.

They describe an action that **does not transfer to an object**.

Structure

Subjekt + Verb

Examples

| Verb | Meaning |
|-----|-----|
| schlafen | to sleep |
| gehen | to go |
| sterben | to die |
| fallen | to fall |
| wandern | to hike |
| wachsen | to grow |
| aufwachen | to wake up |
| verschwinden | to disappear |

Example sentences

Ich schlafe in der Nacht.  
(I sleep at night)

Ich gehe mit meinen Freunden.  
(I go with my friends)

Mein Vater ist gestorben.  
(My father died)

Ich gehe ins Bett schlafen.  
(I go to bed to sleep)

⚠ Important

These verbs **do not take Akkusativ objects**.

---

### 🔹 Transitive Verben

**Definition**

Verben, die **ein Akkusativobjekt** brauchen.

They describe an action that **affects something or someone**.

Structure

Subjekt + Verb + Akkusativobjekt

Examples

| Verb | Meaning |
|-----|-----|
| brauchen | to need |
| haben | to have |
| lieben | to love |
| mögen | to like |
| kaufen | to buy |
| suchen | to search |
| finden | to find |
| lesen | to read |
| machen | to make |
| tragen | to carry / wear |

Example sentences

Ich brauche Hilfe.  
(I need help)

Ich habe ein Auto.  
(I have a car)

Ich liebe dich.  
(I love you)

Ich koche Abendessen.  
(I cook dinner)

⚠ Important

Without an **object**, the sentence often feels incomplete.

</td>

<td valign="top">

### 🔹 Verben, die transitiv und intransitiv sein können

**Definition**

Some verbs can be **both transitive and intransitive**.

They **may take an object but do not have to**.

Examples

| Verb | Meaning |
|-----|-----|
| essen | to eat |
| brechen | to break |
| schreiben | to write |
| fahren | to drive |
| öffnen | to open |
| beginnen | to begin |
| verstehen | to understand |
| lernen | to learn |
| ändern | to change |
| steigen | to rise |

Example sentences

Ich lese gerade.  
(I am reading)

Ich lese gerade ein Buch.  
(I am reading a book)

Er bricht den Stift.  
(He breaks the pen)

Der Stift bricht.  
(The pen breaks)

Ich fahre nach Bremen.  
(I drive to Bremen)

Ich fahre ein Auto.  
(I drive a car)

---

### 🔹 Ditransitive Verben

**Definition**

Verben, die **zwei Objekte** haben können:

• **Dativobjekt** (person)  
• **Akkusativobjekt** (thing)

Structure

Subjekt + Verb + Dativ + Akkusativ

Examples

| Verb | Meaning |
|-----|-----|
| geben | to give |
| schenken | to gift |
| schicken | to send |
| leihen | to lend |
| bringen | to bring |
| zeigen | to show |
| kaufen | to buy (for someone) |
| wünschen | to wish |

Example sentences

Ich gebe Ali ein Buch.  
(I give Ali a book)

Ich bringe euch einen Kaffee.  
(I bring you a coffee)

Ich kaufe meinem Bruder einen Computer.  
(I buy my brother a computer)

Ich gebe einen Computer.  
(I give a computer)

⚠ Important

Person → **Dativ**  
Thing → **Akkusativ**

Example

Ich gebe **Ali** (Dativ) **ein Buch** (Akkusativ).

</td>
</tr>

</table>

</details>


<details>
<summary><b>📌 Ergänzungen – Nominativ und Akkusativ Ergänzung</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Nominativergänzung (Subject)

**Definition**

The **Nominativ** shows **who or what performs the action**.

German question words

| Question | Meaning |
|---|---|
| Wer? | Who? |
| Was? | What? |

Structure

Subjekt + Verb + (Objekt)

Important rule

In questions about the subject, **the verb stays singular**.

Example questions

Wer macht Urlaub in Italien?  
Wer hat Urlaub?  
Wer isst den Apfel?

Example answers

Petra macht Urlaub in Italien.  
Peter und Thomas haben Urlaub.  
Ali isst den Apfel.

More examples

Wer macht einen Apfelkuchen heute?  
Ich mache einen Apfelkuchen.

Wer schläft in meinem Bett?  
Ich schlafe in deinem Bett.

---

### 🔹 Nominativergänzung mit sein / werden / heißen

Some verbs connect the subject with **another nominative element**.

Important verbs

| Verb | Meaning |
|---|---|
| sein | to be |
| werden | to become |
| heißen | to be called |

Question structure

Wer / Was bist / ist / wird ... ?

Examples

Wer ist das?  
Das ist unsere Kursleiterin.

Wer bist du?  
Ich bin ein Freund von Peter.

Was ist gut?  
Das Wetter ist gut.

Was seid ihr von Beruf?  
Wir sind Architekten.

Was wird er?  
Er wird ein Arzt.

Was ist das?  
Das ist ein Bus.

Wie heißt du?  
Ich heiße Talha.

</td>

<td valign="top">

### 🔹 Nach "sein" und "werden"

After **sein** and **werden**, we can use:

• a **Nominativergänzung (noun)**  
• an **adjective**

Meaning of **werden**

werden = something **changes or develops**

Examples

Das ist der neue Lehrer.  
(This is the new teacher)

Der Film war sehr interessant.  
(The film was very interesting)

Sie wird Ärztin.  
(She becomes a doctor)

Am Nachmittag wurde es sehr heiß.  
(In the afternoon it became very hot)

---

### 🔹 Akkusativergänzung (Akkusativobjekt)

**Definition**

The **Akkusativ** shows the **direct object** of the action.

German questions

| Question | Meaning |
|---|---|
| Wen? | Whom? |
| Was? | What? |

Structure

Subjekt + Verb + Akkusativobjekt

Examples

Wen trifft Alina am Donnerstag?  
Am Donnerstag trifft Alina eine Freundin.

Was machen Peter und Thomas?  
Sie machen einen Ausflug.

Was hat Ali in seiner Hand?  
In seiner Hand hat er einen Ring.

---

### 🔹 "es gibt" + Akkusativ

The expression **es gibt** always uses **Akkusativ**.

Meaning

es gibt = there is / there are

Example

Was gibt es im Klassenzimmer?

Es gibt einen Tisch und eine Tafel.

⚠ Important

After **es gibt** → always **Akkusativ**

Example

Es gibt einen Lehrer.  
Es gibt eine Schule.

</td>
</tr>

</table>

</details>


<details>
<summary><b>📌 Dativergänzung & Stellung von Dativ- und Akk. | Dative complement & position of dative and accusative objects</b> <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Dativergänzung (Dativobjekt)

**Definition**

The **Dativ** shows **to whom something is given, shown, or done**.

German question word

| Question | Meaning |
|---|---|
| Wem? | To whom? |

Example question structure

Wem gibt / kauft / zeigt ... ?

---

### 🔹 Examples

Wem kauft Markus die Fußballschuhe?  
Markus kauft **seinem Sohn** die Fußballschuhe.

Wem zeigt Klaus die Stadt?  
Klaus zeigt **seinem Vater** die Stadt.

---

### 🔹 Dativ + Akkusativ together

Often a **Dativ object and Akkusativ object appear together**.

Example sentences

Anita backt **ihrer Mutter** einen Kuchen.  
Frank schreibt **seinen Eltern** einen Brief.

Structure

Subjekt + Verb + **Dativ (person)** + **Akkusativ (thing)**

---

### 🔹 Verben mit Dativ

Some verbs **always use Dativ**.

Examples

Er hilft seinem Vater.  
(He helps his father)

Ich antworte ihm.  
(I answer him)

Harald dankt seiner Schwester herzlich.  
(Harald thanks his sister warmly)

Sie glaubt ihr nicht.  
(She does not believe her)

---

| Artikel | Nominativ | Akkusativ | Dativ |
|---|---|---|---|
| der | er | ihn | ihm |
| die | sie | sie | ihr |
| das | es | es | ihm |
| die (Plural) | sie | sie | ihnen |

Personal pronouns

| Person | Nominativ | Akkusativ | Dativ |
|---|---|---|---|
| ich | ich | mich | mir |
| du | du | dich | dir |
| wir | wir | uns | uns |
| ihr | ihr | euch | euch |
| Sie | Sie | Sie | Ihnen |



</td>

<td valign="top">

### 🔹 Personalpronomen (Nominativ / Akkusativ / Dativ)



### 🔹 Example with two objects

Ich kaufe **meinem Bruder** **einen Computer**.

Variations

Einen Computer kaufe ich meinem Bruder.  
Meinem Bruder kaufe ich einen Computer.

Questions

Wer kauft seinem Bruder einen Computer?  
Ich.

Wem kaufst du einen Computer?  
Meinem Bruder.

Was kaufst du deinem Bruder?  
Einen Computer.




---

### 🔹 Stellung von Dativ- und Akkusativobjekt

**1️⃣ Nomen + Nomen**

First **Dativ**, then **Akkusativ**

Example

Anita schenkt ihrem Sohn ein Buch.  
Ali schenkt seinem Vater einen Computer.

---

**2️⃣ Pronomen + Pronomen**

First **Akkusativ**, then **Dativ**

Example

Er schenkt **ihn ihm**.  
Sie schenkt **sie ihr**.

---

**3️⃣ Pronomen + Nomen**

First **Pronoun**, then **Noun**

Examples

Anita schenkt **es ihrem Sohn**.  
Anita schenkt **ihm das Buch**.

---

### 🔹 More Examples

Alina schenkt ihrer Tochter eine Tasche.  
Sie schenkt sie ihr.

Das Kind schenkt dem Mädchen ein Buch.  
Es schenkt es ihm.

Die Kinder schenken ihren Eltern die Blumen.  
Sie schenken sie ihnen.

---

### 🔹 Quick Question Overview

| Case | Question |
|---|---|
| Nominativ | Wer? |
| Akkusativ | Wen? |
| Dativ | Wem? |

Example

der → den → dem  
wer → wen → wem
</td>
</tr>

</table>
</details>

<details>
<summary><b>📌 Wichtige Verben mit Akkusativ und Dativ (Important Verbs with Accusative and Dative)</b> <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Akkusativ + Dativ Verbs?

Some German verbs take **two objects** in a sentence.

• **Dativ → person**  
• **Akkusativ → thing**

Structure

Subject + Verb + **Dativ (person)** + **Akkusativ (thing)**

Example

Ich gebe **dir** **ein Buch**.  
(I give **you** **a book**.)

---

### 🔹 Abbreviations

| Abbreviation | Meaning |
|--------------|--------|
| jdm. | jemandem (someone – Dativ) |
| jdn. | jemanden (someone – Akkusativ) |
| etw. | etwas (something) |

Example

jdm etw geben → to give someone something

---

### 🔹 Important Verbs with Dativ + Akkusativ

| Verb | Meaning |
|------|--------|
| anbieten (jdm. etw.) | to offer |
| bezahlen (jdm. etw.) | to pay for |
| bestellen (jdm. etw.) | to order |
| bringen (jdm. etw.) | to bring |
| empfehlen (jdm. etw.) | to recommend |
| erklären (jdm. etw.) | to explain |
| erzählen (jdm. etw.) | to tell |
| geben (jdm. etw.) | to give |
| holen (jdm. etw.) | to get |
| kaufen (jdm. etw.) | to buy |
| kochen (jdm. etw.) | to cook |
| leihen (jdm. etw.) | to lend |
| liefern (jdm. etw.) | to deliver |
| mitbringen (jdm. etw.) | to bring along |
| renovieren (jdm. etw.) | to renovate |
| reparieren (jdm. etw.) | to repair |
| reservieren (jdm. etw.) | to reserve |
| sagen (jdm. etw.) | to say |
| schenken (jdm. etw.) | to gift |
| schicken (jdm. etw.) | to send |
| schneiden (jdm. etw.) | to cut |
| schreiben (jdm. etw.) | to write |
| servieren (jdm. etw.) | to serve |
| verkaufen (jdm. etw.) | to sell |
| wünschen (jdm. etw.) | to wish |
| zeigen (jdm. etw.) | to show |

---

### 🔹 Quick Rule

In most German sentences with **two objects**:

**Person → Dativ**  
**Thing → Akkusativ**

Example

Ich kaufe **meiner Mutter** **ein Geschenk**.

Dativ → meiner Mutter  
Akkusativ → ein Geschenk

⚡ Easy Memory

If you can say in English:

**“someone something”**

German usually uses

**Dativ + Akkusativ**

Example

give **someone something**

→ jemandem etwas geben
</td>

<td valign="top">

### 🔹 Example Sentences

anbieten  
Mein Chef hat **mir** drei Wochen Urlaub angeboten.  
(My boss offered **me** three weeks of vacation.)

bezahlen  
Ich bin glücklich, wenn mein Freund **mir** das Essen bezahlt.  
(I am happy when my boyfriend pays **me** for the meal.)

bestellen  
Mein Mann hat **mir** einen Ring bestellt.  
(My husband ordered **me** a ring.)

bringen  
Bring **mir** bitte mein Handy.  
(Please bring **me** my phone.)

empfehlen  
Mein Freund hat **mir** diesen Film empfohlen.  
(My friend recommended this film **to me**.)

erklären  
Kannst du **mir** bitte diese Matheaufgabe erklären?  
(Can you explain this math problem **to me**?)

erzählen  
Meine Mutter hat **mir** immer eine Geschichte erzählt.  
(My mother always told **me** a story.)

geben  
Gib **mir** bitte ein Brot.  
(Please give **me** a bread.)

holen  
Kannst du **mir** ein Glas Wasser holen?  
(Can you get **me** a glass of water?)

kaufen  
Ich kaufe **meiner Mutter** ein Geschenk.  
(I buy **my mother** a gift.)

kochen  
Meine Schwester hat **mir** ein neues Rezept gekocht.  
(My sister cooked **me** a new recipe.)

leihen  
Kannst du **mir** dein Fahrrad leihen?  
(Can you lend **me** your bike?)

liefern  
Die Firma liefert **uns** neue Möbel.  
(The company delivers **us** new furniture.)

mitbringen  
Kannst du **mir** etwas Essen mitbringen?  
(Can you bring **me** some food?)

renovieren  
Der Arbeiter renoviert **uns** das Haus.  
(The worker renovates **our** house.)

reparieren  
Der Mechaniker repariert **mir** das Auto.  
(The mechanic repairs **my** car.)

reservieren  
Ich reserviere **dir** einen Tisch.  
(I reserve **you** a table.)

sagen  
Ich sage **dir** die Wahrheit.  
(I tell **you** the truth.)

schenken  
Ich schenke **meiner Mutter** Blumen.  
(I gift **my mother** flowers.)

schicken  
Ich schicke **dir** eine Nachricht.  
(I send **you** a message.)

schneiden  
Ich schneide **dir** einen Apfel.  
(I cut **you** an apple.)

schreiben  
Ich schreibe **dir** einen Brief.  
(I write **you** a letter.)

servieren  
Der Kellner serviert **uns** das Essen.  
(The waiter serves **us** the food.)

verkaufen  
Er verkauft **mir** sein Auto.  
(He sells **me** his car.)

wünschen  
Ich wünsche **dir** viel Glück.  
(I wish **you** good luck.)

zeigen  
Ich zeige **dir** mein neues Handy.  
(I show **you** my new phone.)



</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Wichtige Regeln für Nominativ, Akkusativ und Dativ im Satz (Important Rules for German Cases in a Sentence)</b> <code>Beginner</code> </summary>

<table>
<tr>
<td valign="top">

### 🔹 Rule 1 — Subject = Nominativ

In every German sentence, the **subject is always in Nominativ**.

The subject answers:

| Question | Meaning |
|---|---|
| Wer? | Who? |
| Was? | What? |

Example

Der Mann kauft ein Auto.

Subject → **Der Mann** (Nominativ)

---

### 🔹 Rule 2 — Dativ is usually a Person

The **Dativ object usually refers to a person**.

It answers the question:

| Question | Meaning |
|---|---|
| Wem? | To whom? |

Example

Ich gebe **dem Mann** ein Buch.

Dativ → dem Mann (person)  
Akkusativ → ein Buch (thing)

⚠ Important

Most of the time

**Person → Dativ**  
**Thing → Akkusativ**

---

### 🔹 Rule 3 — Many Verbs require Dativ

Some verbs **always use the Dativ case**.

Example verbs

| Verb | Meaning |
|---|---|
| helfen | to help |
| danken | to thank |
| folgen | to follow |
| vertrauen | to trust |
| glauben | to believe |
| gefallen | to like |

Example

Ich helfe **dir**.  
(I help you)

Du glaubst **mir**.  
(You believe me)

</td>

<td valign="top">

### 🔹 Rule 4 — Order of Dativ and Akkusativ (Noun + Noun)

If **both objects are nouns**, the order is:

**Dativ → Akkusativ**

Example

Ich kaufe **meinem Bruder** **einen Computer**.

Dativ → meinem Bruder  
Akkusativ → einen Computer

---

### 🔹 Rule 5 — Personal Pronouns (Pronoun + Pronoun)

If **both objects are pronouns**, the order changes:

**Akkusativ → Dativ**

Example

Ich gebe **es dir**.  
(I give it to you)

Structure

Subject + Verb + **Akk Pronoun + Dat Pronoun**

---

### 🔹 Rule 6 — Pronoun + Noun

If one object is a **pronoun** and the other is a **noun**:

The **pronoun comes first**.

Example

Ich gebe **dir das Buch**.

Pronoun → dir  
Noun → das Buch

Another example

Ich gebe **es meinem Bruder**.

---

### 🔹 Quick Summary

| Situation | Order |
|---|---|
| Noun + Noun | Dativ → Akkusativ |
| Pronoun + Pronoun | Akkusativ → Dativ |
| Pronoun + Noun | Pronoun → Noun |

---

### 🔹 Easy Memory Trick

Think like this:

Person → **Dativ**

Thing → **Akkusativ**

Example

Ich schenke **meiner Mutter** **Blumen**.

Dativ → meiner Mutter  
Akkusativ → Blumen

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Nebensatz mit „wenn“ und „als“ (Subordinate Clauses with “when”)</b> | <code>Beginnner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Temporalsätze (Time Clauses)

**Temporalsätze** describe **time**.

They answer questions like:

| Question | Meaning |
|---|---|
| Wann? | When? |
| Seit wann? | Since when? |
| Bis wann? | Until when? |

They can describe:

• a **specific time**  
• a **repeated event**  
• a **duration**

---

### 🔹 Nebensatz Rule

In a **Nebensatz (subordinate clause)** the **verb goes to the end**.

Structure

Hauptsatz + Nebensatz

Example

Ich war traurig, **als ich krank war**.

---

### 🔹 „als“ — one time in the past

„als“ describes an event that happened **once in the past**.

Examples

Als ich das erste Mal in Deutschland war, war das Wetter nicht sehr gut.  

Eine Musikgruppe hat gespielt, als wir das Städtchen besichtigt haben.

Meaning

When I was in Germany for the first time, the weather was not very good.

---

### 🔹 „wenn“ — repeated events in the past

„wenn“ describes something that happened **many times in the past**.

Often used with

| Word | Meaning |
|---|---|
| immer | always |
| jedes Mal | every time |

Examples

Immer wenn Jan aus dem Wasser gekommen ist, musste er sich in der Sonne aufwärmen.

(Jedes Mal) Wenn Jan das Zelt zusammenfalten wollte, hat es sich wieder geöffnet.

Jan musste sich immer in der Sonne aufwärmen, wenn er aus dem Wasser gekommen ist.

Das Zelt hat sich jedes Mal wieder geöffnet, wenn Jan es zusammenfalten wollte.

</td>

<td valign="top">

### 🔹 „wenn“ — present or future

„wenn“ is also used for events in the **present or future**.

The **wenn-clause can come before or after the main clause**.

Examples

Ich erhole mich am besten, **wenn ich faulenze**.  
(I relax best when I do nothing.)

Wenn das Wetter morgen schön ist, **(dann)** machen wir einen Ausflug.  
(If the weather is nice tomorrow, we will go on a trip.)

Wenn ich von der Schule nach Hause komme, koche ich mir einen Kaffee.

Ich will diese neuen Schuhe kaufen, wenn sie im Angebot sind.

⚠ Important

If the **wenn-clause comes first**, the main clause can start with **„dann“**.

Example

Wenn das Wasser kocht, **dann** entsteht Dampf.

---

### 🔹 Replacement with „bei“

Sometimes **wenn + present/future** can be replaced with:

**bei + Dativ**

Example

Wenn ich morgen einkaufe, darf ich Kaffee nicht vergessen.

→ Beim Einkaufen morgen darf ich Kaffee nicht vergessen.

Meaning

When I go shopping tomorrow, I must not forget coffee.

---

### 🔹 Position of Nebensatz

The subordinate clause can be **before or after** the main clause.

1️⃣ Nebensatz first

Wenn ich Zeit habe, gehe ich spazieren.

2️⃣ Hauptsatz first

Ich gehe spazieren, wenn ich Zeit habe.

⚠ Important

Verb always goes **to the end of the Nebensatz**.

---

### 🔹 Quick Comparison

| Word | Use | Example |
|---|---|---|
| als | one time in the past | Als ich Kind war |
| wenn | repeated past | Wenn ich krank war |
| wenn | present / future | Wenn ich Zeit habe |

---

### 🔹 Easy Memory Trick

Use **ALS** → one past event  

Use **WENN** →  
• repeated events  
• present  
• future  

Example

Als ich klein war, spielte ich Fußball.

Wenn ich Zeit habe, spiele ich Fußball.

</td>
</tr>
</table>

</details>


======= EXTRA CONTETNT GOES HERE =========

<details>
<summary><b>📌 sehr vs zu – Degree of Adjectives</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Basic Difference

| Word | Meaning | Example |
|-----|-----|-----|
| sehr | very | Der Film ist sehr interessant. |
| zu | too / excessively | Der Film ist zu langweilig. |

✔ **sehr = positive intensity**  
✔ **zu = more than necessary (problem)**

---

### 🔹 Using **sehr**

Structure:

Subject + Verb + **sehr + Adjective**

Examples:

- Das Essen ist **sehr gut**.  
- Deutsch ist **sehr interessant**.  
- Ich bin **sehr müde**.

Meaning → **very**

---

### 🔹 Using **zu**

Structure:

Subject + Verb + **zu + Adjective**

Examples:

- Der Kaffee ist **zu heiß**.  
- Das Auto ist **zu teuer**.  
- Ich bin **zu müde**.

Meaning → **too much / problematic**

</td>

<td valign="top">

### 🔹 Meaning Difference

Example:

- Das Wasser ist **sehr heiß**.  
→ very hot (ok)

- Das Wasser ist **zu heiß**.  
→ too hot (problem)

---

### 🔹 Important Rule

> ✔ **sehr = positive degree**  
> ✔ **zu = excessive degree**

Comparison:

- Der Film ist **sehr lang**.  
- Der Film ist **zu lang**.

---

### 🔹 Structure Formula

Subject + Verb + **sehr / zu + Adjective**

Examples:

- Das Buch ist **sehr interessant**.  
- Das Buch ist **zu schwierig**.

---

### 🔹 Example Sentences

- Der Test war **sehr schwer**.  
- Das Wetter ist **sehr kalt**.  
- Der Kaffee ist **zu stark**.  
- Das Haus ist **zu teuer**.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 zu + Adjektiv + um...zu – Result Structure</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Structure

| Pattern | Example |
|-------|-------|
| zu + Adjektiv + um...zu | Das Auto ist zu teuer, um es zu kaufen. |

Meaning:

**too ... to ...**

Example:

- Das Wasser ist **zu heiß, um es zu trinken**.

---

### 🔹 More Examples

- Der Text ist **zu schwer, um ihn zu verstehen**.  
- Ich bin **zu müde, um zu arbeiten**.  
- Das Haus ist **zu klein, um dort zu wohnen**.

</td>

<td valign="top">

### 🔹 Formula

Subject + Verb + **zu + Adjective**,  
**um + zu + Verb**

Example:

Das Problem ist **zu schwierig, um es zu lösen**.

---

### 🔹 Meaning

This structure shows:

**something is too much to do something**

Example:

Der Kaffee ist **zu heiß, um ihn zu trinken**.

Translation:

The coffee is **too hot to drink**.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 sehr vs viel vs viele vs sehr viel</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Basic Difference

| Word | Used With | Meaning |
|-----|-----|-----|
| sehr | adjectives / adverbs | very |
| viel | uncountable nouns | much / a lot |
| viele | plural nouns | many |
| sehr viel | strong amount | very much |

---

### 🔹 sehr

Used with **adjectives or adverbs**

Examples:

- Der Film ist **sehr interessant**.  
- Ich bin **sehr müde**.  
- Deutsch ist **sehr schwierig**.

---

### 🔹 viel

Used with **uncountable nouns**

Examples:

- Ich habe **viel Arbeit**.  
- Wir trinken **viel Wasser**.  
- Er hat **viel Geld**.

</td>

<td valign="top">

### 🔹 viele

Used with **plural nouns**

Examples:

- Ich habe **viele Freunde**.  
- Es gibt **viele Autos**.  
- Viele Studenten lernen Deutsch.

---

### 🔹 sehr viel

Means **very much / very large amount**

Examples:

- Ich arbeite **sehr viel**.  
- Ich lerne **sehr viel Deutsch**.

---

### 🔹 Quick Rule

| Word | Use |
|-----|-----|
| sehr | adjectives |
| viel | singular mass nouns |
| viele | plural nouns |

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 so...dass – Result Clause</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Structure Overview

| Pattern | Example |
|------|------|
| so + Adjective + , + dass | Der Test ist so schwer, dass ich ihn nicht verstehe. |

Meaning:

**so ... that**

---

### 🔹 Examples

- Das Wetter ist **so kalt, dass** wir zu Hause bleiben.  
- Ich bin **so müde, dass** ich sofort schlafe.  
- Der Film ist **so spannend, dass** alle ihn mögen.

</td>

<td valign="top">

### 🔹 Rule

✔ „dass“ introduces a **Nebensatz**  
✔ Verb goes to **end**

Example:

Der Kaffee ist **so heiß, dass ich ihn nicht trinken kann**.

---

### 🔹 Structure Formula

Main clause + **so + adjective**,  
**dass + subject + object + verb**

Example:

Der Film ist **so interessant, dass ich ihn zweimal sehe**.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 zu vs genug – Degree of Adjectives</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Basic Difference

| Word | Meaning | Example |
|-----|-----|-----|
| zu | too | Der Kaffee ist zu heiß. |
| genug | enough | Der Kaffee ist heiß genug. |

---

### 🔹 zu

Means **more than necessary**

Examples:

- Das Auto ist **zu teuer**.  
- Der Film ist **zu langweilig**.

</td>

<td valign="top">

### 🔹 genug

Means **sufficient**

Structure:

Adjective + **genug**

Examples:

- Der Kaffee ist **warm genug**.  
- Ich bin **alt genug**.

---

### 🔹 Comparison

- Das Wasser ist **zu kalt**.  
- Das Wasser ist **warm genug**.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Adjective Position – Quick Rule</b> | <code>Absolute Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 Predicate Adjective

When adjective comes **after verb**

Structure:

Subject + Verb + Adjective

Examples:

- Das Haus ist **groß**.  
- Der Film ist **interessant**.

✔ No adjective ending

</td>

<td valign="top">

### 🔹 Attributive Adjective

When adjective comes **before noun**

Structure:

Article + Adjective + Noun

Examples:

- ein **großes** Haus  
- ein **interessanter** Film

✔ Adjective ending required

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 dieser / welcher – Demonstrative & Question Articles</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are these?

| Word | Meaning |
|-----|-----|
| dieser / diese / dieses | this / that |
| welcher / welche / welches | which |

✔ Used **before nouns**  
✔ They change form depending on **case + gender**

---

### 🔹 Nominativ

| Gender | dieser | welcher |
|------|------|------|
| der | dieser Mann | welcher Mann |
| die | diese Frau | welche Frau |
| das | dieses Kind | welches Kind |
| plural | diese Leute | welche Leute |

Example:

- **Welcher Film** ist gut?  
- **Dieser Film** ist interessant.

---

### 🔹 Akkusativ

| Gender | dieser | welcher |
|------|------|------|
| der | diesen Mann | welchen Mann |
| die | diese Frau | welche Frau |
| das | dieses Kind | welches Kind |
| plural | diese Leute | welche Leute |

Example:

- **Welchen Film** siehst du?  
- Ich sehe **diesen Film**.

</td>

<td valign="top">

### 🔹 Dativ

| Gender | dieser | welcher |
|------|------|------|
| der | diesem Mann | welchem Mann |
| die | dieser Frau | welcher Frau |
| das | diesem Kind | welchem Kind |
| plural | diesen Leuten | welchen Leuten |

Example:

- **Mit welchem Freund** gehst du ins Kino?  
- Ich spreche mit **diesem Mann**.

---

### 🔹 Genitiv (less common in A2)

| Gender | dieser | welcher |
|------|------|------|
| der | dieses Mannes | welches Mannes |
| die | dieser Frau | welcher Frau |
| das | dieses Kindes | welches Kindes |
| plural | dieser Leute | welcher Leute |

---

### 🔹 Important Pattern

These follow **almost the same pattern as „der“ articles**

| der-word | dieser-word |
|------|------|
| der | dieser |
| den | diesen |
| dem | diesem |

---

### 🔹 Structure Formula

Question:

**Welcher + Noun + Verb ?**

Example:

Welches Buch liest du?

Answer:

**Dieser / Diese / Dieses + Noun**

Example:

Dieses Buch ist sehr interessant.

---

### 🔹 Example Sentences

- **Welcher Bus** fährt nach Wien?  
- **Dieser Bus** fährt nach Wien.  
- **Mit welchem Lehrer** lernst du Deutsch?  
- Ich spreche mit **diesem Lehrer**.

</td>
</tr>
</table>

</details>

<details>
<summary><b>📌 Sentence Connectors – übrigens, jedoch, inzwischen, trotzdem, außerdem</b> | <code>Beginner</code></summary>

<table>
<tr>
<td valign="top">

### 🔹 What are Sentence Connectors?

These words **connect ideas between sentences**.

✔ They often appear in **position 1**  
✔ Verb comes in **position 2**

Example:

Heute habe ich viel Arbeit. **Außerdem habe ich einen Termin.**

---

### 🔹 übrigens

Meaning:

**by the way**

Examples:

- **Übrigens**, morgen ist ein Feiertag.  
- Ich habe dich gestern gesehen. **Übrigens**, wie geht es dir?

Used when adding **extra information**

---

### 🔹 jedoch

Meaning:

**however**

Examples:

- Ich möchte kommen, **jedoch** habe ich keine Zeit.  
- Der Film ist gut, **jedoch** sehr lang.

Shows **contrast**

---

### 🔹 außerdem

Meaning:

**besides / furthermore**

Examples:

- Ich lerne Deutsch. **Außerdem** lerne ich Englisch.  
- Das Restaurant ist günstig. **Außerdem** ist es sehr gut.

Adds **additional information**

</td>

<td valign="top">

### 🔹 inzwischen

Meaning:

**meanwhile / in the meantime**

Examples:

- Ich koche das Essen. **Inzwischen** deckt sie den Tisch.  
- Er arbeitet. **Inzwischen** wartet seine Familie zu Hause.

Shows **something happening during the same time**

---

### 🔹 trotzdem

Meaning:

**nevertheless / despite that**

Examples:

- Es regnet. **Trotzdem** gehe ich spazieren.  
- Ich bin müde. **Trotzdem** arbeite ich weiter.

Shows **unexpected result**

---

### 🔹 Quick Comparison

| Word | Meaning |
|-----|-----|
| übrigens | by the way |
| jedoch | however |
| außerdem | besides |
| inzwischen | meanwhile |
| trotzdem | nevertheless |

---

### 🔹 Structure Rule

Connector + **Verb + Subject**

Example:

**Trotzdem gehe ich** zur Arbeit.

(not: Trotzdem ich gehe)

---

### 🔹 Example Sentences

- Ich habe wenig Zeit. **Trotzdem arbeite ich weiter.**  
- Ich lerne Deutsch. **Außerdem lese ich Bücher.**  
- **Übrigens komme ich morgen vorbei.**  
- **Inzwischen arbeitet er im Büro.**

</td>
</tr>
</table>

</details>